...
**Goal:** Give a child a task-scoped durable Session with multiple activation Runs and exact lineage while preserving existing child history.
**Architecture:** Host-owned binding and idempotent operation key in both SQL backends; Service creates Session/Run under parent authority; Journal is the observable lifecycle.
**Tech Stack:** Go, SQLite/Postgres migrations, existing storage conformance.
**Spec:** [architecture](../../specs/2026-09-23-issue39-eino-orchestration-design.md), R1/R2/R6; [index](index.md) owns predecessor ORCH-01 and status.
