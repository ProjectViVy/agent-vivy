...
First action: inspect Service Run/Resume authority boundary, graph fixture and pinned Eino checkpoint API; record origin parent Run, current authorizer Run, activation Run, ChildSession, graph Run, node Run, tool-call and checkpoint identities. Test a real brokered Service call. If Service lacks a viable test entrypoint or Workflow cannot recover, record the missing seam and stop. No standalone-lambda success claim or second execution loop. Planning approval does not bypass G0/G1. Use frozen D4/D8/D10/D12/D14 contracts in the index; this proof does not implement them.
