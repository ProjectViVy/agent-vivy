...
**Architecture:** An internal Service activation consumes ORCH-02's durable binding; the Eino agent runner handles model/tool turns, while Service owns all Run, approval, event and budget edges.
...
- [ ] Reuse Service admission, model and tool selection, policy and approval/question handling. Convert worker manager into thin lifecycle/orchestration glue. Propagate parent cancellation and Session deletion via Service; existing `child/cancel` and live `WaitChild` continue to work. Pin task mask hint as immutable instruction using current prompt assembly, no catalog inheritance or new persona record.
