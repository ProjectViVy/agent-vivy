...
**Scope gate:** [D8](../../specs/2026-09-23-issue39-eino-orchestration-design.md) is unresolved. `child/followup` creates a new activation Run; it does not meet the earlier #39 candidate for delivering bounded messages to an active child. Do not add or silently exclude an inbox API until the product scope is dispositioned. Peer-to-peer swarm traffic is a separate deferred question.
...
- [ ] Before implementing any child messaging endpoint, disposition D8 with the product owner. If direct mailbox delivery is selected, add a separately reviewed contract and tests for sender/recipient authority, deterministic ordering, delivery/acknowledgement state, parent/child interruption and restart, and bounded retention/redaction. If scope is explicitly limited to later-Run follow-up, record that as an accepted scope decision; the earlier candidate must not be described as implemented.
