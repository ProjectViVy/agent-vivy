...
| [T9](T9.md) | Core-3 | R3,R4 | T4: atomic ContinuityStore admission and idempotent receipts | Safe file presentation and verified transfers | Accepted | Implementation `f9d8f4b7`: `runtime.SensitiveWorkspacePath` extracted as the shared sensitive-path classifier (rpc callers delegate inward); `openWorkspaceFileSecure` is the os.Root-bound safe open — pre-Lstat irregular/symlink reject, EvalSymlinks preflight, post-read recheck; `DeliverableService.Present` fingerprints ≤32MiB/128MiB into committed `deliverables.presented` sets via T4 receipts; `Read` opens a per-owner single-slot transfer whose first read hashes the bounded file into a scratch snapshot (mismatch → `changed`, temp deleted), serving ≤256KiB sequential chunks bound to opaque process-scoped `xfr_` handles with 60s idle TTL; `DeleteSession` closes session transfers via `ServiceDeps.Deliverables`. 14 focused tests PASS, `go test ./internal/rpc` PASS, `just ci` PASS on `f9d8f4b7` (conformance digest `27b6b2b8…`), recorded in `verification.md`. |
| [T10](T10.md) | Core-3 | R1,R3,R4 | T9: secure presentation and connection-bound verified bytes; T3: sanitized HistoryService, model tools and inspection RPC | Presentation tool, RPC and durable discovery | Accepted | Implementation `89d71bde`: `present_files` effectful tool (strict `files[]{path,description}`+`title` schema, set_id result) through the governed adapter; `deliverables/list|get|read|close` RPCs on trusted connection identity with stable `status: reason` error envelope and gated capability; `DeliverySetPage` moved to domain; `SetViewStores` drops rewound-hidden sets; `history/search` `artifact_id` matches committed set/item provenance inside `deliverables.presented` events (task_id stays unsupported); app wires service over `dataRoot/transfers`, registry, `ServiceDeps`/`ControlDeps`. Focused tests: tools 6 + rpc 6 + runtime 18 PASS (incl. restart recovery, plan-deny, transport read, hidden-turn exclusion, artifact scoping), `git diff --check` clean, `just ci` PASS on `89d71bde` (digest `00d1b7b4…`), recorded in `verification.md`. |
| [T11](T11.md) | Core-3 | R3,R5 | T10: present tool/RPC, durable list and artifact history filter; T8: reference/get adapter, timeline row and detail surface | Additive delivery groups and GUI actions | Accepted | Implementation `6a0064f6`: `deliverable-download.ts` owns digest-bound chunked reads (transfer closed on mismatch/abort), sha256-verified object-URL download, ≤4KiB availability probe, ≤64KiB text preview with HTML/SVG excluded from in-app rendering; `deliverables.presented` events fold to immutable `DeliverableCard` groups (no card before commit); FilesPanel Deliveries section reads the same store and focuses the original group — separate from the modified-file list; one active transfer per item via AbortController map; sdk/ui Face mirrors all new api ops + store fields (compat test green); i18n en/zh + cross-face contract updated. Focused tests 74/74 PASS, typecheck PASS, `git diff --check` clean, `just ci` PASS on `6a0064f6`, real-browser smoke 6/6 incl. byte-identical download and changed-state re-verify, recorded in `verification.md`. |
