...
| [T3](T3.md) | Core-1 | R1 | T2: stable HistoryQueryStore cuts and backend parity | Governed history projection, tools and inspection RPC | Review pending | T2 is accepted at source/review level (`669e496`; implementation/fix range through `cf6aebc`). T3 implementation commits `3519c73`, `bcc6aaa`, `60fb0a4` are present; `git diff --check` passes. The planned Go focused test is unverified (exit 127: `go` not found), the real SQLite fixture and several required focused cases are not yet present, and no independent subagent review artifact returned. PostgreSQL/just/CI remain unverified. |
