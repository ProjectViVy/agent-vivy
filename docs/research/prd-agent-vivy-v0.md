...
> **Revision note (2026-09-16, supersedes the counts in D-018 / D-022 / D-023).**
> The provider set is now **three** bundles: `deepseek` (the **default**),
> `openai` (OpenAI-compatible) and `anthropic`. DeepSeek entered exactly
> through the D-019 path this table requires — a product decision, delivered
> as a pre-baked bundle (`fixtures/provider/deepseek.yaml`, re-derived under
> D-025 with `provenance.entry: deepseek`) — not as a runtime plugin.
> `active: deepseek`, `DEEPSEEK_API_KEY`, model `deepseek-flash`, base
> `https://api.deepseek.com` (no `/v1`; the endpoint is
> `https://api.deepseek.com/chat/completions`). D-023's exclusion list is
> therefore reduced by one entry (`deepseek` is now carried); every other
> Diva entry it names remains excluded. The original rows are left unchanged
> as the historical record. See
> `docs/logs/2026-09-16-deepseek-default-provider/`.
...
- v0.6 (2026-09-16): Admitted **DeepSeek** as the third and **default** provider bundle through the D-019 capability-proposal path (`active: deepseek`, `DEEPSEEK_API_KEY`, model `deepseek-flash`, base `https://api.deepseek.com` with no `/v1`). Superseded the provider counts in D-018/D-022/D-023 by revision note; the original rows are retained as the historical record. `openai` and `anthropic` remain available as optional bundles. Record: `docs/logs/2026-09-16-deepseek-default-provider/`.
