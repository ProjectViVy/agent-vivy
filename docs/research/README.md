...
| 14a | `agent-vivy/docs/architecture/VIVY-CHANNEL-PACK.md` | **Adopted 2026-08-30.** Super-channel Host in kernel; this batch of adapters are `plugins/` with `seam: channel`; config cannot grow new adapters; A2A/NeuroLink reserved. Native Eino A2A = steal `eino-ext/a2a` codec, not `RegisterServerHandlers` | When discussing Telegram/Feishu/QQ/DingTalk/Discord, A2A, NeuroLink, Eino A2A, or "built-in plugins" |
| 14c | `agent-vivy/docs/architecture/VIVY-CHANNEL-EVOLUTION.md` + `docs/plans/channel-epic/` | EPIC evolution tree + per-slice PLAN pack for successor agents. Host package `internal/channelhost`. Index: `docs/TODO.md` §0.2.8 | When a sub-agent picks CH-C1..C7 or needs the architecture tree |
