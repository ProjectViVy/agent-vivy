...
> 并给出移植 / 暂缓 / 拒绝三类判定与理由。本文只做对照与结论，不做实现。
...
- **移植（首批）**：summary-only 奖励轮、空输出兜底、循环内微压缩、
  循环可观测事件。这四个机制都落在 Eino 的 middleware 接缝上，已有可行性
  验证，且直接补上"循环可能空转 / 上下文可能超限 / 循环不可观测"三个缺口。
- **暂缓**：context-overflow 反应式重试、admission 限流 / 熔断、运行时控制
  通道、plan 阶段能力矩阵。前两者受 Eino 接缝与单用户形态约束，需要更细的
  设计或收益证据；后两者是独立的大切片，适合作为后续换代候选。
- **拒绝**：MEMRULES 写门控、ACTMEM 脉冲 / 复盘、媒体处理、artifact 工具
  结果、经验日志。这些是 agent-diva 的记忆与介质机制，属于 Vivy 的 MEM-1
  记忆切片，不是 AGENT-LOOP 的回合生命周期问题，且与"模型可见 ≡ 已记录"
  的 Journal 不变式冲突。

一句话结论：**Vivy 不移植 agent-diva 的循环骨架（stage-contract turn 管线），
它移植 agent-diva 在循环内部积累的四个治理机制。** 骨架不同（Eino ReAct vs
自研 stage），机制相同（预算、压缩、兜底、可观测）。
...
- **暂缓**：机制本身有价值，但受接缝约束、收益证据不足、或依赖独立切片
  （换代 / 记忆 / 多租户）先行。
- **拒绝**：属于其他切片（记忆 MEM-1、媒体、多用户基础设施），或与 Vivy
  不变式冲突，或纯属 agent-diva 形态（单用户桌面 vs 个人网关）不需要。

---

## 3. Eino v0.9.13 接缝可行性（移植前提）
...
1. **`BeforeModelRewriteState` 每次模型生成前运行**（`adk/chatmodel.go`），
   ...
   （D4）的落点：检测到预算/空转条件时，下一轮只给空工具面，模型只能输出
   总结，不调工具。
2. **`SetRunLocalValue` / `GetRunLocalValue`**（`adk/react.go`）存 gob 可序列化
   值，跨中断/恢复存活，工具执行上下文可读——这是"跨迭代记账"（D3 变体）
   与"压缩已发生"标记的落点。
3. **`AgentOutput.CustomizedOutput` + `adk.SendEvent`**（`adk/interface.go`）
   ...
   **循环可观测事件**（loop.summary_pass / context.compacted）的落点，需要
   在 mapper 里新增一类自定义事件分支。
...
| Vivy 现状 | 无。`MaxToolTurns` 用尽即 `ErrExceedMaxIterations` → `run.failed`（`engine.go:98-103`，`service.go` terminalEvent）。预算到了，模型连"我已经做了这么多，这是结论"都说不出口 |
...
agent-diva 在上下文超限时重建一次再跑。Vivy 的 `buildRunContext` 已在**回合入口**
按预算截断（`context.go`），回合内超限的概率比 agent-diva 低（agent-diva 是长
会话连续循环，Vivy 每回合重装配）。真正的回合内超限（一次工具返回巨大结果）
已被 `MaxToolResultBytes` 单点拦截。**暂缓理由**：收益证据不足，且"重建一次"
在 Eino 里意味着在 `BeforeModelRewriteState` 里改写整段历史（可行但需要验证
对 checkpoint/恢复的副作用），应等 P3 微压缩上线后看残留缺口再决定。

#### D2. admission 限流 / 熔断 / 执行冲突

agent-diva 是 100 次/小时限流 + 拒绝熔断 + 执行冲突检查。Vivy 是**单用户个人
网关**，天然低频，且已有预算账本（`budget.go`：MaxEvents/MaxModelCalls/
MaxToolCalls/MaxRetries）与预检（`preflight.go`）在入口处限制。**暂缓理由**：
限流阈值对个人网关是伪参数；若未来出现多租户或 Studio 批量评测，再以
`budget.go` 为底座扩展，不单独移植。

#### D10. 运行时控制通道（StopSession/ResetSession/CompactSession/…）

agent-diva 有循环内控制命令。Vivy 的控制面在 **RPC 层**（`internal/rpc/control.go`：
run/interrupt、approval、question、review、child），实现的是"回合外控制"（中断、
审批、恢复），不是"循环内命令"。两者职责不同：Vivy 的 RPC 控制面已覆盖人闸
需求（approval/question/review），缺的是"运行中压缩/改预算/停止会话"这类命令。
**暂缓理由**：这是一个独立切片（新 RPC 方法 + 运行中干预通道 + 恢复语义），
且依赖 P3/P1 先把循环内治理做实，否则控制通道没有可控制的对象。列入换代候选。

#### D12. plan 阶段工具能力矩阵

agent-diva 按 TurnMode（Agent/Plan/Ask）给 fail-closed 工具白名单。Vivy 已有
**按 profile 的政策引擎**（`policy.go`：default→Prompt、plan→Deny、
read_only→Deny、full_auto→Allow），工具面在 InvokableRun 门处逐工具 Evaluate
（`tooladapter.go`），plan 阶段整体 Deny 而非按工具白名单。**暂缓理由**：Vivy
的"plan 阶段不给工具"是比"plan 阶段给指定工具"更严格的现状，安全目标已达成；
细化到 per-phase 白名单需要"运行中切换实际可调工具集"（只能走 `adk.TurnLoop`
或逐轮重建 agent，见 §3-5），是骨架级改动。列入换代候选，先不做。
...
| D13 | MEMRULES 写门控 | 记忆机制，属 MEM-1 记忆切片；不是回合生命周期问题 |
| D14 | ACTMEM 脉冲/复盘 + 闲置折叠 | 同上；且"会话后沉淀"与 Journal"模型可见 ≡ 已记录"不变式需要专门设计 |
| D15 | 媒体处理 | 介质能力（图片等），Vivy 当前模型面为文本；属独立能力切片 |
| D16 | canonical 工具结果（sha256 artifact 引用） | 与 Journal 逐事件回放 / 工具结果内联于 `tool.finished` 的事件模型冲突；改动横跨 domain/storage/RPC/UI，收益对个人网关不足 |
| D17 | 经验日志 | 记忆机制，归 MEM-1；且需要独立的经验检索面 |
| D18 | token 预算账本 | **不是拒绝**：Vivy 已有对等的 `budget.go` 预算账本（MaxModelCalls/MaxToolCalls/MaxEvents），方向一致，无需移植，后续按需对齐口径即可 |
| D8 | 预算化上下文装配（DEFERRED 分层） | Vivy 的 `buildRunContext` 已按字节/条数截断并配对工具消息（`context.go`），骨架不同但目标已达成；不移植分层，仅吸收"预算进装配"的思想（P3 依赖） |
| D11 | plan 批准生命周期屏障（stop_after_AwaitingApproval） | Vivy 已有对等：审批中断挂起 + `ResumeWithParams` 恢复（`mapper.go` extractInterrupt、`service.go` handleInterrupt），目标已达成 |
...
### 5.1 首批（turn 生命周期治理批）——本次调研之后唯一建议立即做的切片

| 项 | 内容 | 落点 |
|---|---|---|
| P1 | summary-only 奖励轮 | 新 middleware（`BeforeModelRewriteState` + run-local 计数器 + `ToolInfos=nil`） |
| P2 | 空输出兜底 | `mapper.go` 最终消息分支 + `service.go` consume 分类 |
| P3 | 循环内微压缩 | 新 middleware 复用 `internal/runtime/compaction`（`CompactToolResult`/`Pruner`） |
| P4 | 循环可观测事件 | `mapper.go` 自定义事件分支 + domain 新增 `loop.summary_pass`/`context.compacted` |
| 配套 | `config.yaml` runtime 新增 `loop.*` 开关（summary 奖励轮开关/阈值、微压缩开关） | `internal/config/config.go` + `config.example.yaml` |
...
- plan 阶段工具能力矩阵（D12）：骨架级（TurnLoop），列换代候选。
- admission 限流（D2）：仅在多租户 / 批量评测出现时以 `budget.go` 为底座扩展。

### 5.3 明确不做

- 记忆族（D13/D14/D17）→ MEM-1 记忆切片。
...
- 实现批（§5.1）单独开迭代日志，遵循 `just ci` 门与 UI 冒烟门。
