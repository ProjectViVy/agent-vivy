...
- State: `COMPLETE · 2026-09-12`; depends on P2. The implementation landed
  with PR #18, and the dedicated closure lane completed the missing Task 7
  whole-envelope proof, bound approval authority to canonical post-Middleware
  arguments, and corrected the redacted approval projection.
...
- [x] Write `TestEveryToolPathUsesHostInvoke`; expected RED is that built-in and
  plugin paths enter different adapters.
- [x] Write duplicate static/dynamic ID, discovery timeout, and schema-hash
  change tests.
- [x] Implement one catalog with deterministic namespaces and dynamic cache
  invalidation.
- [x] Adapt Eino `components/tool` only in `internal/runtime`; cite the concrete
  pinned API in the iteration verification.
- [x] Remove direct Tool execution and duplicate registries.
- [x] Run `go test ./internal/toolhost ./internal/runtime -run Tool`.
- [x] Commit `refactor(tool): centralize toolhost dispatch`.
...
- [x] Write table tests asserting all 11 exact IDs are T1, default-on, and
  rejected from any T2 source.
- [x] Write `TestMinimalRecipeCanOmitButCannotReplaceProtectedTool`.
- [x] Convert each protected Tool constructor to the focused SDK contract
  without changing behavior.
- [x] Make source/Trust, not a second Port, distinguish internal Tools.
- [x] Run existing focused tests for filesystem, patch, multiedit, bash,
  ask-user, and skills Tools.
- [x] Commit `refactor(tool): protect core tool identities`.
...
- [x] Write RED tests for path escape, undeclared Secret enumeration,
  unapproved process spawn, HTTP host/scheme/port escape, and `rpc.client`
  misuse as general network.
- [x] Implement scoped capabilities without handing out raw Journal, Policy,
  DB, Eino, or credential-store handles.
- [x] Preserve error cause chains while redacting Secret content.
- [x] Run `go test ./internal/modulehost ./sdk/internal/assembly -run Grant`.
- [x] Commit `feat(plugin): enforce scoped module host grants`.
...
- [x] Write `TestRewriteRevalidatesSchemaPolicyAndGrant`; expected RED proves the
  current path cannot consume a public ordered chain.
- [x] Write tests for identity rewrite, recursive execution, timeout, panic,
  invalid decision, and multiple rewrites.
- [x] Execute the exact Port Catalog order and fail closed on every Middleware
  failure.
- [x] Keep Kernel Policy as final authority even when Middleware passes.
- [x] Run `go test ./internal/toolhost -run Middleware`.
- [x] Commit `feat(tool): add governed pre-tool middleware`.
...
- [x] Write RED tests proving a Run Observer sees only committed events and can
  receive a duplicate stable event ID.
- [x] Write tests proving diagnostic overload increments a visible drop counter
  without blocking a Run.
- [x] Remove mutation-capable Observer callbacks and old optional type
  assertions.
- [x] Ensure Observer failure cannot roll back or change Tool results.
- [x] Run `go test ./internal/observerhost ./internal/runtime -run Observer`.
- [x] Commit `feat(observer): separate run and diagnostic delivery`.
...
- [x] Write `TestStatusReadDoesNotStartOrProbeProvider`; expected RED is any
  lifecycle effect caused by inspection.
- [x] Implement per-provider timeout, count bound, namespace, and unavailable
  projection.
- [x] Convert LSP status directly to `std/status-source@v1`.
- [x] Run `go test ./internal/statushost ./internal/app ./internal/rpc -run Status`.
- [x] Commit `feat(status): host read-only module status`.

### Task 7: Prove the complete Tool envelope

**Files:**

- Modify: `internal/app/assembly_governance_e2e_test.go`
- Modify: `sdk/internal/assembly/evidence.go`
- Modify: `internal/runtime/tooladapter_test.go`
- Modify: `internal/runtime/tooladapter.go`
- Modify: `internal/runtime/mapper.go`
- Create: `internal/runtime/approval_binding.go`
- Create: `internal/runtime/approval_binding_test.go`
- Modify: `internal/runtime/policy_context.go`
...
- [x] Run the same request through all four source classes and assert identical
  schema, Policy, Middleware, approval, execution, result-bound, and Journal
  stages.
- [x] Assert a rewritten request cannot reuse an approval for old arguments.
- [x] Assert Secrets are absent from error and Observer payloads.
- [x] Run `go test ./internal/toolhost ./internal/runtime ./internal/app`.
- [x] Run `just ci` and record the Tool envelope trace.
- [x] Commit `test(tool): prove one governed execution envelope`.
