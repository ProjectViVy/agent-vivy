...
**Goal:** Replace the PG-D1 adapter and history blockers with source-backed, executable decisions.
...
- Executable probes and backend tests are recorded in the execution evidence linked from the authoritative index.

## Contracts and prerequisites

Consumes: PG-D1 sections 2, 8 and 9; pinned Eino v0.9.13 and repository rules.

Produces: PG-D2: exact Eino prompt/review/stop API; message-to-work-event ordering anchor; history/counter rules; shared bounds and config source. Probe evidence in the execution log.
...
## Task 3: Human admission response and lifetime

Use the existing authenticated `turn/start` caller contract. A request is a process-local admission waiter, not a durable queued Run or request ticket: return only after a real RunID is committed, return the existing busy conflict when another primary is already committed, and let the caller retry. Register the human intent before waiting on the per-session startup gate so an uncommitted Goal candidate yields. Cancellation before the gate opens writes no message or run; after the durable run commit, the existing detached-run lifetime applies.

- [x] Add a concurrency probe that registers a human request while the session gate is held and verifies the Goal round remains uncharged.
- [x] Linearize human intent registration and the final atomic Goal admission commit through a per-session intent gate.
- [x] Document the exact waiter, conflict, cancellation and disconnect semantics in PG-D2; no accepted RunID is fabricated before storage commit.
...
Execution record: see [2026-09-23 verification](../../logs/2026-09-23-plan-goal-foundation/verification.md). The Eino review/resume, human-admission and SQLite history probes passed, and the direct equivalent of `just ci` passed. Live PostgreSQL remains a separate gate; an unset VIVY_POSTGRES_TEST_DSN is not evidence.
...
Return the tested revision, changed file list, each scenario result, logs, deviations and remaining blockers. The exact Eino, history and synchronous human-admission contracts are now executable. The index retains the broader product gates before downstream release.
