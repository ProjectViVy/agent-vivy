...
> **交付情况（2026-09-07 收尾）**：本分支（`feat/tui-detail-polish`，worktree
> `agent-vivy-tui-polish`）交付批次 A 全部（F2 `73aa834`、F3 `7f60072`、
> F4 `b93af3e`、F11 `02781d8`）与批次 B 全部（F1 `8dbf8ff`、F12 `15c7950`、
> F6 `a345a51`）。执行期间另一 lane 已在 main 并行交付同一提案的 F1/F12
> （`e2ad7f2`）、F5/F9/F13（`9052ca5`，merge `b108f0c`）、F10（`2768390`）、
> F7（`83d242e` + 审计修复 `bb5e294`）。本分支 F1/F12 与 main 版本互为平行
> 实现（保留在分支、不合并；如落地应取 main 版本并只移植 F6 与批次 A）；
> F6 与批次 A 在 main 无对应实现，为本分支独有交付。批次 C/D 不在本分支
> 重复实现。
...
### F2 多行输入完整可见 + 编辑器自适应高度（已完成 73aa834）
...
### F1 动画 spinner + 计时器（已完成 8dbf8ff；main 另有平行实现 e2ad7f2）
...
### F12 状态行信息增强（左会话/右环境，含队列数）（已完成 15c7950；main 另有平行实现 e2ad7f2）
...
### F5 工具卡展开/收起（main lane 已交付 9052ca5，本分支不重复实现）
...
### F9 推理（reasoning）块折叠（main lane 已交付 9052ca5，本分支不重复实现）
...
### F13 空会话欢迎态（hero）（main lane 已交付 9052ca5，本分支不重复实现）
...
### F10 侧栏 Modified Files 配色 + 路径截断（main lane 已交付 2768390，本分支不重复实现）
...
### F7 终端窗口标题（main lane 已交付 83d242e + 审计修复 bb5e294，本分支不重复实现）
