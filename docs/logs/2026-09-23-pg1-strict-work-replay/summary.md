...
Fix round 2 keeps restart Goal recovery fail-closed across the entire bounded replay: it remembers an admission candidate but does not restore authority until every later page has validated. It adds a real SQLite regression with a valid first page and a corrupt second page; the storage replay contract is unchanged.
