...
5. A host-authenticated connection bound to one session can enter Plan for that session; an attempt to enter Plan for another existing session is rejected with no Work event. The first reviewed `start_goal` choice through the same authenticated `plan/decide` RPC wakes the Goal driver before origin terminal cleanup, then admits one round after origin completion. An identical replay creates no second decision or round.
