...
- History messages and Work events use durable sequence anchors. SQLite exercised same-timestamp ordering, fork and rewind; PostgreSQL has matching implementation and shared conformance cases, but live PostgreSQL was not run in this record. Fork and rewind preserve the source history and do not refund spent Goal rounds or copy approval authority.
...
The six mask failures were `TestMaskCatalogEmbedsCanonicalBuiltIns`, `TestMaskCatalogResolverAndOwnedReturns`, `TestMaskCatalogDuplicateIDsFailClosed`, `TestServiceListMergesBuiltinsAndCustoms`, `TestServiceCaptureResolvesBuiltinsAndOwnsCopies`, and `TestServiceSelectionUsesResolvedCaptureAndRejectsReservedLookup`. Each reported `mask catalog: validate builtin/programmer: definition body is not normalized` (the duplicate-ID assertion instead reported that error where it expected a duplicate-ID error). This task made no mask changes. The first full CI run at this gate-repair revision was not a pass. A second `just ci` was started after the digest refresh, then canceled before completing on supervisor instruction because the focused reproduction had passed and the unrelated mask failure was already recorded. A later clean-checkout CI pass after the LF rule is recorded in [the masks prompt EOL log](../2026-09-23-masks-prompt-eol/verification.md), and a full `just ci` at current candidate `a258feb` is recorded in [the 2026-09-24 PG-1 RPC ownership log](../2026-09-24-pg1-plan-submit-rpc-ownership/verification.md); these later results supersede the red gate for current-tip readiness without changing the historical result above.
