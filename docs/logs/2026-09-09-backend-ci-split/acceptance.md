...
1. Open the stacked pull request's **Checks** view.
...
3. Confirm `backend ci` runs the plugin fixture validator, builds `ui/dist`,
   ...
   even when `ui ci` reports the known I18N assertion failures.
4. Confirm the aggregate status is named `just ci` and succeeds only when both
