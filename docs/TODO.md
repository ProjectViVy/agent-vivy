...
| TUI-MD-STREAM-CACHE | 流式 Markdown 的 stable-prefix 增量渲染 | OPEN | 本轮只对 live bubble 全量 glamour 重绘；Crush `streamingMarkdown` 的空白行边界缓存未移植。长流式答案可能在每 tick 重绘当前气泡。相关路径 `sdk/tui/view/markdown.go`、`.workspace/crush/internal/ui/chat/streaming_markdown.go`（只读参考）。 |
