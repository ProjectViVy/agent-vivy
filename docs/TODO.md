...
| TUI-STREAM-N4 | 收敛 `model.completed.content` 与单事件 payload 上界的契约冲突 | OPEN | 2026-09-04 增量流复审确认 delta/reasoning 已按 `max_event_payload_bytes` 无损拆分，但 `model.completed` 仍携完整重组答案，可能超过“单事件 payload”名义上界；直接截断会破坏 assistant history，直接拆 completed 会改变单终结语义。应设计“durable delta 为正文 + completed 为结束元数据”或独立 blob/message 引用迁移，并保持旧 Journal 兼容。相关路径 `internal/runtime/mapper.go`、`service.go:appendAssistantMessage`、wire schema。 |
...
| TUI-STREAM-N6 | tool-call 前导 assistant 文本进入 durable Message 历史 | OPEN | 2026-09-04 mapper 边界复审确认：工具前导文本以 `model.delta` 留在 Journal/实时 TUI，`tool.requested` 正确封闭显示轮次，但 `persistAndPublish` 只把 `model.completed` 镜像进 Message store；会话终结后由 `session/messages` 重载时该前导文本不可见，也不会进入下一轮 `runMessages`。本轮不把额外 completion 冒充 provider 轮次（会提前关闭 trajectory）；后续应与 N4 的 delta-canonical/message-ref 协议一起设计可重建的 Message 投影。相关路径 `internal/runtime/service.go`、`mapper.go`、`context.go`。 |
...
| TFLAKE-CRON-AT-DISABLE | `TestCronAtJobDisablesAfterRun` 在 full-suite 负载下 5s 超时 | OPEN | 2026-09-04 live-stream lane 的最终 `just ci` 连续两次只在该测试失败；失败快照已完成 `Enabled=false`/`NextRunAtMs=0`，但异步 run 尚未在 5s 内写回 `LastStatus=ok`。隔离重跑 1.275s 通过；排除该金丝雀后的 `go test ./...`、headless、plugin/face 全绿。与已关闭的 delete-after-run `TFLAKE-CRON` 是同类负载敏感接线测试，后续应改为 30s 金丝雀或确定性 settle 契约测试。相关路径 `internal/runtime/cron_scheduler_test.go:165`。 |
...
| TUI-PARITY-2 | VIVY CODE 次级 Crush 呈现控制：split diff 切换、图片附件入口、模型档位与 token/cost 状态展示 | OPEN | 2026-09-04 思考档位已闭环：shared `/thinking [auto\|on\|off]` + Ctrl+T、`session/context.thinking_supported` 真值门、`turn/start.thinking`、排队快照、切换到不支持会话自动降级，见 `docs/logs/2026-09-04-vivy-code-thinking-mode/`。本轮已闭环图片附件（built-in/packed fullscreen、legacy REPL、project-relative server seam、history/queue/failure 语义），见 `docs/logs/2026-09-04-vivy-code-image-attachments/`。仍缺 split diff、模型选择与更完整 token/cost 呈现；无权威 RPC/能力位的控件继续隐藏，不得以 demo 假控件替代。相关路径 `sdk/tui/`、`internal/tui/`、`faces/tui/`。 |
...
| TUI-SIDEBAR-N1 | 补齐 Crush 右栏所需的会话更新时间与运行上下文真值 | OPEN | 2026-09-04 sidebar/session slice 已移除伪造内容并接入真实 `session/context`；同日 freshness 子片闭环：每个终态回合刷新 active context，异步结果按 request generation + session ID 防串会话，权限更新同步 active session，`--prompt` 建会话不再丢失既有列表；shared rail 现展示真实 run/queue、feed/total messages、compaction trigger/summary，见 `docs/logs/2026-09-04-vivy-code-sidebar-freshness/`。当前 control plane/session DTO 仍没有独立 `updated_at`、cwd、有效 model/provider、session cost、聚合 modified files、LSP 健康或 session-mounted MCP/skills 投影，因此这些区块保持隐藏。应先扩展有权威来源的 RPC/contract，再接入 shared `sdk/tui/view`，不得把 CreatedAt 或进程 cwd 重命名冒充 UpdatedAt/工作区。 |
...
| TUI-CMD-N3 | 动态 user-invocable skills 与 MCP prompts 进入命令面板 | OPEN | 2026-09-04 静态 slash registry 面板已交付，但当前 skills 只有 catalog/view，MCP 只有 configured/resources/read；尚无带参数/描述/可调用性真值的 session-mounted skill/MCP prompt catalog。应先扩展 typed control-plane contract，再合并为动态 palette rows，不得从文件名或配置猜测可调用命令。相关路径 `sdk/tui/command`、`sdk/tui/view`、skills/MCP RPC。 |
