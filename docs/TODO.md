...
| RB-1 | 回退调研：回退怎么做、到底支不支持代码回退 | OPEN | 调研完成 2026-08-31（`docs/research/rollback-capability-research-2026-08-31.md`）：① Vivy 现状不支持任何代码回退，且当前形态下"不支持"是对的——文件工具只写每 run 隔离 scratch（`workspace_root`/`<runID>`），checkpoint 桥纯运行态无文件语义，旧内容只有 32KB 有损 diff 残影；② 承诺口径 = 文件级回退（version chain），**不做 git 语义回退**（git 归用户，agent 只对自己动过的文件负责）；③ 落点 = 主线内核（存储+写路径+审批三处核心），L1 存档+单文件恢复随 VC-1（bash 化后成为安全网前提），L2 会话级批量回退随 VC-3；④ 参照发现：Crush 版本链全库无 restore/undo 消费方（仅 TUI 展示面板）——Vivy 做了即领先。待拍板 O1..O6（研究 §6：保留策略/存储形态/存档范围/恢复治理/bash 边界），拍板后本行转 DONE。eino v0.9.13 原生核查（研究 §7）：checkpoint/中断恢复原生且 Vivy 已消费（纯运行态零文件语义）；文件版本/回退、git 语义原生均无——L1/L2 须 Vivy 自建，挂载 seam = Vivy 已实现的 `filesystem.Backend`（Write/Edit 调用点即挂链位） |
...
| UI-CRON-P2 | cron 外发通道/补跑/at 表单入口 | OPEN | 2026-08-30 cron 闭环后留白：`payload.deliver/channel/to` 仅存储不生效（Vivy 无外发通道，等 CH-0）；错过的调度不补跑（catch-up）；`at` 类型后端支持但 UI 表单未暴露；cron 运行沿用会话默认审批策略，无独立治理面 |
...
| SKILL-MKT-1 | 市场技能的更新/升级路径 | OPEN | 2026-08-30 与 DIVA 同为 create-only（重装 409，删除后重装）；无版本比对与原地升级 |
| SKILL-MKT-2 | DIVA `always` 常驻注入未移植 | OPEN | 2026-08-30 Vivy 走 Eino skill 中间件 inline 加载；frontmatter `always`（全文常驻注入 + 4k/2k 预算）需内核能力提案 |
| UI-E2E-STALE | `just ui-e2e` 两条既有规格断言已过期文案（非本次改动引入） | OPEN | `runtime.spec.ts:84` 断言「密钥只由运行环境管理」、`welcome-wizard.spec.ts:33` 断言「API 密钥通过运行环境变量注入，不在界面中填写或保存。」——两串文案在 `ui/src` 均已不存在，密钥提示曾被 i18n 改写（现文案见 `ui/src/i18n/zh.ts` 的 `catalogKeyHint`/`secretNote` 等键）。2026-08-27 e2e 复跑确认：新增的 `language-setting.spec.ts` 通过，这两条为过期规格失败，与语言分区改动无关；需按现文案同步断言后恢复全绿，本次范围外未改。2026-09-01 复跑 `just ui-e2e`：4 失败 / 4 通过 / 1 跳过——① runtime.spec 卡 `getByRole('button', { name: '附件' })`（附件入口在 VC-1g 后不在 ChatInput 默认面，需按现 UI 改定位）；② welcome-wizard 仍卡「配置模型」步骤（同上过期文案）；③ language-setting `filter({ hasText: 'EN' })` strict 撞车——hasText 为大小写不敏感子串，顶栏「OpenAI | gpt-4o-mini」模型切换钮含 "en"，与 English 卡片两元素；④ model-refresh `getByText('gpt-4o-mini', { exact: true })` 撞顶栏同名 span（模型元数据/成本片在顶栏展示当前模型后新增）。已用 stash 本片 UI 文件在干净 HEAD 复跑 runtime+wizard 证实与自动标题改动无关（同为 2 failed）。修法：①③④收窄选择器按现 UI 同步，②按现文案同步断言 |
