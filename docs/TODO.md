...
| TUI-PARITY-2 | VIVY CODE 次级 Crush 呈现控制：split diff 切换、图片附件入口、模型档位与 token/cost 状态展示 | OPEN | 思考档位、图片附件、全局模型选择与审批 diff 已闭环。2026-09-05 diff 面直接消费 Journal/RPC 的 bounded unified preview，宽屏默认 split、窄屏 unified，支持 `t` 切换、`f` 全屏、纵横滚动、增删统计、双栏行号及 Crush 决策键；非文件 mutation 不从任意文本猜 diff，built-in/packed 共用一套 projection/view。见 `docs/logs/2026-09-05-vivy-code-split-diff/`。仍缺更完整 token/cost 呈现；无权威 RPC/能力位的控件继续隐藏。相关路径 `sdk/tui/`、`internal/tui/`、`faces/tui/`。 |
