...
| MCP-TRANSPORT-1 | MCP stdio 传输与 OAuth 走上游 Eino/mcp-go 组件接入，不自研协议栈 | OPEN — slice 1 DONE 2026-09-08; slice 2 OAuth OPEN | stdio 切片已贯通 config/settings/app/runtime/RPC/browser import-export/i18n：`transport.NewStdioWithOptions` + `client.NewClient` 懒启动；配置即授权（PATH 名或绝对路径、危险 denylist，不新增 execute allowlist）；`env_from` 为 CHILD→HOST 引用；cwd 钳制到 `runtime.workspace_root`；缺失 env 显式 error、进程死亡下一操作检测、stdio 不自动重启；EinoExt `GetTools` 与现有治理路径不变。TUI/sidebar 复用既有 configured/initialized/error 三态，并加法投影 `transport`/`env_missing`，不新增独立 UI/surface 协议。未完成/另记：raw-frame 上界、Windows process-tree 回收、OAuth TokenStore/needs-auth/RPC。提案与验收：`docs/plans/2026-09-07-mcp-stdio-upstream.md`、`docs/logs/2026-09-08-mcp-stdio/`；VC-4 行中 stdio/OAuth 条目由本行承载。 |
| MCP-STDIO-RAW-FRAME | 为 stdio JSON-RPC 增加 raw-frame 字节上界 | OPEN | mcp-go v1.0.0 stdio reader 以 `ReadString('\n')` 读帧，当前只保证 decoded/projected bounds；需上游 PR 或可验证的 transport seam，不能把投影上界当 raw-frame 上界。关联 `docs/plans/2026-09-07-mcp-stdio-upstream.md` 限制⑤。 |
| MCP-STDIO-PROCESS-TREE | Windows stdio 子进程树治理 | OPEN | 当前 `CommandFunc` 设置 `WaitDelay` 并由 mcp-go 关闭当前 child，但没有子进程树回调/Job Object 语义；后续需先锁定树回收、取消和超时合同，再实现并做 Windows smoke。关联 `docs/plans/2026-09-07-mcp-stdio-upstream.md` 限制⑩。 |
...
| PLG-1 | 进阶插件联盟：将架构层统一为 `Module + Port + Generated Assembly`，`internal` 与 `pluggable` 共用 descriptor/依赖图/lifecycle/Generation provenance，但使用不同 trust ceiling；产品层仍按 loop/world/tool/provider/channel/face 真名展示。第一批零行为迁移 Tool/ToolWorld/Channel/Face；随后补齐 `provider/llm`、typed pre-tool middleware、context/skill source、run observer、`ui/slot` 的 Definition/Provider/Consumer/failure model。`SeamProvider` 目前仍有名无实；源码已是五 seam + 11 Grants，旧合同存在漂移。单一 `Service.Run`、Journal authority、policy/approval/Grant、domain event、Assembly compiler 必须 kernel-coupled；Eino 只作 internal 编排内核。编译期 pack/generation 不变，不做 runtime Go loading；不可信代码走 MCP/sidecar | OPEN | 2026-09-08 新调研 `docs/research/advanced-plugin-alliance-2026-09-08.md`（Eino/Hermes/DeepSeek Harness 主对照，OpenFang/ZeroClaw/OpenClaw/Pi 旁证，P0-P5 与 conformance matrix）；此前 `docs/research/plugin-model-dsh-vs-vivy-research-2026-09-02.md`、`docs/research/plugin-ui-dsh-implementation-research-2026-09-02.md` 继续作为历史输入。当前仅研究提案，实施未授权 |
...
| Date | Item | Note |
|---|---|---|
| 2026-09-07 | TUI-VIEWPORT-N1-OPEN 打开态视口收口（锚定 + 渲染缓存 + 加载态） | 暂停视口从纯数字偏移升级为 `(segment,line)` 消息锚点 + assembly 指纹：`clampChatScroll` 仅在指纹变化（内容变更）时把锚点重映射为当前偏移，锚点上方工具结果展开不再漂移视口，同内容 clamp 不覆盖显式偏移；`chatSegments` 以 maphash 全字段指纹复用分段 assembly（分隔行内建），`renderChat` 只 slice 可见窗口 join，不再每帧重渲染+全量 flatten 整段历史（mdCache 在重建路径兜单消息渲染）；`surface.Meta.Loading` 由 Live 在 delete→switch→load 窗口置位，空会话视图渲染“正在加载会话历史…”，不冒充空对话 hero。assembly 指针 `New()` 分配一次原地更新，值拷贝共享。Filing: `docs/logs/2026-09-07-tui-viewport-n1/`. |
| 2026-09-07 | TUI-CMD-N5 动态命令 P2 parity 与资源清理 | 展开 RPC 真取消：surface 新增 `DynamicCommandCanceller`，`Live` 以 view 私有 request id 注册 cancel（幂等，RPC 结束注销），view 在 Esc 与 active session 切换时先取消 RPC 再丢弃迟到结果（编辑器不再等满 15s 超时；取消 msg 走既有 discard 路径恢复 draft + 诊断）；参数表单记录 `dynamicArgumentSession`，session 变化即关闭旧 session 表单；palette↔表单互斥复核由既有键路由优先级保证，无需新增；`applyDynamicCommandsMsg` 成功路径清除遗留 `dynamic commands:` 前缀错误；`<loaded_skill>` 信封评估后保持等价纯文本（有意等价，无行为收益）。plain REPL 目录缺口随单一 fullscreen face 重构消失（palette 每次打开即刷新）。Filing: `docs/logs/2026-09-07-tui-cmd-n5/`. |
| 2026-09-07 | TUI-MD-TOOL-RESULTS 工具卡片正文分流 Markdown/chroma/diff | 移植 crush 工具结果路由：`toolResultContent` 四分类（JSON 走字节保真 `json.Indent` 重排；`isUnifiedDiffContent` 行前缀 diff 判定 = crush diffdetect 移植 + hunk-alone Vivy 兼容，修复 markdown 列表被误判为 diff 的子串缺陷；markdown 嗅探沿用 crush 模式表；其余 plain），`renderToolBodyLines` 将 JSON/markdown 包为 fence 代码块经既有 `renderMarkdown`（`quiet=false`，normal 样式才带 chroma 配置）渲染，chroma 高亮复用共享渲染器而非自建 SyntaxHighlight；diff 走既有 `renderDiffBody` 着色；plain 保持 `wrapText`。compact 8 行截断与 `ctrl+o` 展开不变。Filing: `docs/logs/2026-09-07-tui-md-tool-results/`. |
| 2026-09-07 | TUI-MD-STREAM-CACHE 流式 Markdown stable-prefix 增量渲染 | 移植 crush `streamingMarkdown` 空白行边界缓存至共享渲染器：新增 `sdk/tui/view/streaming_markdown.go`（(id,quiet) 键控、上限 8 条、宽度变更/非前缀改写重置；fence 奇偶/HTML/链接引用/松散列表续行/setext/table pipe 危险源拒绝，任何疑虑回退全量渲染并保留缓存），`renderMessageBody` 对 `Streaming` 气泡走增量路径，完成态仍走原全量渲染+mdCache；`renderMarkdown` 拆出 `renderMarkdownLocked`，流式序列全程持 `mdRenderMu` 防共享 goldmark 状态交错。测试覆盖边界提升/无边界 flush/开 fence 不切/宽度变更与改写重置/危险源表/Streaming 气泡走缓存。Filing: `docs/logs/2026-09-07-tui-md-stream-cache/`. |
| 2026-09-07 | TUI-CMD-N4 动态命令异步状态后续收口 | 新增 `dynamicCommandEpoch`：`bootCmd` 取目录前捕获 epoch，`applyBoot` 仅在 epoch 匹配时应用 commands/CommandErr，晚到 boot 不再覆盖已完成 refresh 的目录或复活旧错误；展开成功但 `Driver.Send` 返回 nil 时恢复原 slash draft 并提示；pending expansion 锁编辑器与次级 surface（Esc 恢复 draft 取消、Ctrl+C 退出），mouse 路由复核无绕行。覆盖 boot-vs-refresh（目录+错误双场景）、send-nil、pending 编辑/overlay 测试。Filing: `docs/logs/2026-09-07-tui-cmd-n4/`. |
| 2026-09-07 | TUI-STREAM-N5 durable stream 防御性契约收紧 | `stream.Decode` 必填 `subscription_id`/`run_id`；`streamRun` 对 after_seq 已覆盖终态或 run 不存在的订阅立即释放（依据 emitTerminal 先落终态事件后翻转状态的提交顺序不变量）；移除隐藏 replay fence 的 `Inbox.Take()`。streamLost epoch 与共享 Inbox 迁移已被 Live 重构吸收；`Live.Close` 所有权边界复核维持原设计。Filing: `docs/logs/2026-09-07-tui-stream-n5/`. |
| 2026-09-07 | TUI-WORKSPACE-OWNERSHIP workspace RPC 所有权校验 | `workspace/list`、`workspace/read` 在触碰文件系统前经 `Runs.GetRun` 证明 run 属于本 Journal（loopback 信任传输下 Journal 存在性即归属事实），未知 run 404 fail closed；`WorkspaceFiles` 从 `Ensure` 切换为 `Existing`（绝不创建目录）并新增 `ErrWorkspaceNotFound`→404 映射；`Read` 改为 open 句柄 Stat + 有界读取的竞态硬化；补控制字符/`..`/分隔符 run id 与 final-component symlink 测试。Filing: `docs/logs/2026-09-07-workspace-ownership/`. |
| 2026-09-07 | §0.1 看板清理：完成行归档 §10.1 | 开放看板只留 OPEN/DEFERRED/WONT-DO/SUPERSEDED/RESOLVED-DEFERRED 行；DONE 与 RESOLVED 行原样移入 §10.1 存档（不删不改写，履行“不许静默删除”规则）。docs-only。 |
...
### 10.1 §0.1 完成行归档（2026-09-07 清理，原样保留）
...
| TUI-MD-TYPOGRAPHY | Crush 对齐的 TUI Markdown 排版 | DONE 2026-09-05 | 助手/用户气泡经 `github.com/charmbracelet/glamour` v1 渲染（非 Charm v2）；标题药丸、列表 `•`、引用 `│`、行内 code chip、围栏 chroma、链接分层与 QuietMarkdown 思考块；已完成消息按 session/width 缓存。见 `docs/logs/2026-09-05-tui-markdown-typography/`。 |
...
| TUI-COMPOSER-HINTS | 底栏居左、切换模式文案、圆角变色 | DONE 2026-09-06 | 快捷键居左且 `shift+tab 切换模式`；智能/计划/只读对应青绿/金/蓝边框。见 `docs/logs/2026-09-06-tui-composer-hints/`。 |
