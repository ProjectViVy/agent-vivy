...
| UI-CHAT-ACT | 消息编辑 / 回退 / 分叉启用 + 截断式重新生成 | OPEN | 2026-08-26 功能栏已移植（复制启用 + 重新生成=重发上一条用户输入开新回合，见 `docs/logs/2026-08-26-chat-message-actions/`）；编辑/回退/分叉为禁用占位。2026-09-02 **设计片交付**：`docs/architecture/JOURNAL-REWIND-AND-FORK.md`——逻辑截断标记（migration 020，不删行、读时折叠，压缩先例同族）+ `session/rewind`/`session/fork` 两 RPC + busy 闸 + UI 接线；实施切分 R1（内核回退）/R2（fork+编辑接线）/R3（打磨），§8 公开问题 3 条待实现时拍板。实现未排期 |
