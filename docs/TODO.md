...
| UI-TRAJECTORY-DEMO | Dashboard 轨迹 Tab 仍为纯演示数据 | OPEN | 2026-09-01 Dashboard 概览接真实 RPC 时确认：`TrajectoryPanel` 自述"纯演示数据，无后端"（`components/trajectory/trajectory-demo-data.ts`，复刻 DeepSeek Harness TrajectoryView 布局）。真实化需内核提供结构化轨迹查询源（`run/log` 有事件流，但演示面板的时间轴/账本/回合折叠形态需要回合级结构）——先提内核能力提案，再换 `api.ts` 真实 RPC |
