...
| MCP-TRANSPORT-1 | MCP stdio 传输与 OAuth 走上游 Eino/mcp-go 组件接入，不自研协议栈 | OPEN | 2026-09-07 拍板：用上游现成组件——mcp-go v1.0.0 `client.NewStdioMCPClient`/OAuth handler + eino-ext/tool/mcp v0.0.9 `GetTools` 传输无关接缝；上游缺口走 PR 途径，不 fork。切片 1 = stdio（config 信封 + `newServer` 分支 + spawn 治理钩子 + 投影层预算 + 进程死亡 fail-closed）；切片 2 = OAuth（TokenStore + needs-auth 状态 + 交互授权 RPC，启动前拍板远程回调 localhost 限制）。提案：`docs/plans/2026-09-07-mcp-stdio-upstream.md`（Eino 能力检查实据、上游限制 9 条、PR 途径与流程要求均在提案内）；VC-4 行中 stdio/OAuth 条目由本行承载。 |
