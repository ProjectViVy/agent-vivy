...
| RB-1 | 回退调研：回退怎么做、到底支不支持代码回退 | OPEN | 2026-08-31 用户立项。范围：checkpoint 桥（FR-8 会话恢复）/ 文件版本 history（VC-3 编辑前存档）/ git 语义回退——三者边界与组合；产出 = Vivy 是否承诺"代码回退"能力及落点（主线/插件/code face），结论回写本行与 VC-3 |
