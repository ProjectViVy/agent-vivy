...
| TUI-SIDEBAR-N1-OPEN | 右栏后续真值扩展与输入设备 | OPEN | 本轮未实现 LSP 健康、session-mounted MCP/skills 投影或鼠标滚轮；它们仍缺权威 control-plane contract/当前架构鼠标入口，后续先建 typed RPC 与双面测试，禁止以配置/进程状态猜值。 |
