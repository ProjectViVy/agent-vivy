...
| TUI-MD-TOOL-RESULTS | 工具卡片正文走 Markdown/chroma 而非纯 wrap | OPEN | 本轮刻意保持 compact tool card + unified diff 着色；Crush 会对工具输出做 quiet markdown / code / diff 分流。不阻塞排版交付。相关路径 `sdk/tui/view/render.go` `renderToolWithOptions`。 |
