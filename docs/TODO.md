...
| TT-2 | 中断 run 的 resume 不恢复挂载集 | DONE 2026-09-02 | `resumeRun` 现在从 `pendingRun.mounted` 取回挂起时捕获的 `MountedTools` 并重新绑定到 resume ctx（审批与提问两条路都覆盖）；restart 恢复场景挂载本就只存内存，回退为空注册表，恢复段内 `skill_view` 仍可再挂载（`docs/logs/2026-09-02-tt2-resume-mounts/`）。挂载历史审计已由 TT-3 `tool.mounted` 补齐；跨重启恢复注册表仍需 TT-1（会话级 pin） |
