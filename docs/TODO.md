...
| TUI-CMD-N2 | 为 `!` shell 与 `@` workspace reference 提供受治理的本地命令协议 | OPEN | 2026-09-04 `@file` 已完成：shared parser/view、built-in/packed fullscreen 与 legacy REPL、ProjectRoot+`os.OpenRoot` bounded text-only resolver、canonical symlink/sensitive/ADS 防护、durable FileContext snapshot/history metadata、SQLite/Postgres edit/fork/delete、context/compaction/request-digest 计量和失败草稿恢复均落地（`docs/logs/2026-09-04-vivy-code-shell-file/`）。`!` 目前仅有 parser/client `shell/start` scaffold，服务端不存在该方法，help 明示 unavailable，且客户端不读文件、不启动进程、不把脚本发给模型。仍待 `runtime.Service.RunShell` 完整 bash 治理（ValidateArgs/Safety/policy/hooks/approval/Journal/resume/cancel/terminal/redaction/output bound）。相关路径 `sdk/tui/command`、`internal/rpc/control.go`、`internal/runtime/`、`internal/tui/`、`faces/tui/`。 |
