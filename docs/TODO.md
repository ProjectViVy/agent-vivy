...
| TUI-STREAM-N3 | 补齐 `model.completed` 的非流式最终文本投影 | OPEN | 2026-09-04 流链审计确认 shared `stream.Interpret`/`Projection` 只靠 `model.delta` 建答案，未消费 `model.completed.content`；正常 streaming provider 已有 delta，但纯非流式/特殊适配器可能只发 completed。后续须以 seq 去重语义接入，避免已有 delta 后重复全文。相关路径 `sdk/tui/stream/events.go`、`reducer.go`。 |
...
| TUI-TOOL-N1 | TUI tool 卡片按 `tool_call_id` 配对而非工具名 | OPEN | 2026-09-04 流链审计确认 notice 丢弃 `tool_call_id`，`tool.finished` 仅按工具名逆向匹配；同名并发调用可能更新错卡。需贯通 shared stream notice/surface card，并同步 built-in/packed 测试。 |
