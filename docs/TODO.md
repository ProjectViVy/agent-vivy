...
| TUI-ATTACH-HARDEN | 附件路径解析补齐 Windows ADS、敏感路径 denylist 与读取后 identity 复核 | OPEN | 2026-09-04 governed-shell 复审发现的既有附件边界：`internal/rpc/attachments.go` 当前未显式拒绝冒号/ADS 路径，也没有敏感路径 denylist 和读取后的 canonical identity 复核。该问题独立于 `!shell` 执行链，需专门安全波次覆盖 Windows 与 TOCTOU 回归；本轮不混改附件协议。 |
