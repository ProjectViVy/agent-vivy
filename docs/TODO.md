...
| SET-RMW | settings 跨 handler 的 Load→modify→Save 仍是 last-writer-wins | OPEN | 2026-09-01 `fileMu` 只串行化文件 I/O 窗口，不覆盖跨 handler 的读改写：两个 handler 各自 Load 后 Save 会丢一次更新（e2e 曾以此为根因之一）。未来演进 `settings.Update(path, fn)` 一类读改写原子接口（内核侧持锁完成 Load→fn→Save），调用方逐步迁移 |
