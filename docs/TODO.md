...
| TUI-CMD-N1 | 暴露高级 TUI 命令 `/compact`、`/fork`、`/rewind`、`/todos`、`/stats`、`/skills`、`/mcp` | OPEN | 2026-09-04 首批共享 parser/registry 与安全命令已交付；这些命令虽有部分 RPC，但仍缺统一 fullscreen/REPL 结果面、参数/确认语义，且 cost、MCP/skills 状态必须保持真实，禁止先挂假成功。相关路径 `sdk/tui/command`、`sdk/tui/view`、`internal/tui/repl.go`、`internal/tui/live.go`、`faces/tui/live.go`。 |
