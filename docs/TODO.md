...
| FACE-TUI-1 | Packed `faces/tui` organ (F3) | OPEN | 2026-08-29 探路客户端 `vivy tui`（`internal/tui`）已能连驻留网关对话/审批；不是配方器官；默认双击仍是 web。2026-09-02 复核前置：F0 已采纳（D2）且 paperwork 已补（`docs/logs/2026-09-02-face-pack-f0-paperwork/`）；headless **功能**已以 `vivy run`（D11）存在但未做成配方器官。2026-09-02 **F1 交付**（`docs/logs/2026-09-02-face-tui-1-f1/`）：`WithoutGateway()` 组合拆掉 HTTP 监听耦合（origin policy/mux/embed/httpServer 仅默认组合构建，`vivy_headless` 随之零监听）+ `App.DialControl` 进程内 JSON-RPC（net.Pipe 复用既有协议/transport 层，零协议改动）；app 级测试钉死"无 embed 跑完对话+审批"与"无 UI 审批挂起 → run/cancel 持久取消"两条（含只读工具审批闸恒自动放行、手工 config 需显式 WorkspaceRoot/审批过期两个钉子）。**剩余** = FaceHost/SDK Face 契约 + 出厂 `faces/tui` 器官 + pack `face:` 配方键——按合同 PR 计划分片实施。**§14 四问已拍板（2026-09-02，全取推荐值，F2/F3 前置解除）**：faces/ 独立 go.mod、默认永远 face: web、第一刀不同居、headless 审批失败退出（`docs/logs/2026-09-02-face-pack-14-rulings/`） |
