...
| TUI-PARITY-2 | VIVY CODE 次级 Crush 呈现控制：split diff 切换、图片附件入口、模型档位与 token/cost 状态展示 | OPEN | 思考档位与图片附件已闭环。2026-09-05 又交付 Crush 式全局模型选择：`Ctrl+L`、`/model [filter]`、预置 bundle + 自定义 registry 权威目录、运行启动栅栏、built-in/packed 同面与选择后右栏刷新，见 `docs/logs/2026-09-05-vivy-code-model-picker/`。仍缺 split diff 与更完整 token/cost 呈现；无权威 RPC/能力位的控件继续隐藏，不得以 demo 假控件替代。相关路径 `sdk/tui/`、`internal/tui/`、`faces/tui/`。 |
