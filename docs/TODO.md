...
|---|---|---|---|
| EINO-BOUNDARY-AUDIT | 复核 Vivy 自研与 Eino/EinoExt 原生能力的边界 | OPEN — MCP SLICE DONE 2026-09-06 | MCP slice 已按 capability check 完成：runtime 使用 Eino MCP `GetTools` 做官方 tools/schema 转换，mcp-go v1.0.0 typed Streamable HTTP 承载 resources/prompts/lifecycle；Vivy 的 mcp_list_tools/mcp_call、PrepareMCPCall、isError、untrusted/bounds 与 settings/app lifecycle 保留。Eino 当前未覆盖 resources/prompts/lifecycle，故 typed plumbing 暂不删除；stdio/OAuth/continuous listening 未做。tool_search、Sequential Thinking、plantask 兼容面及 stream observer callback PoC 仍是本 track 的其他候选，track 不关闭。记录：`docs/research/eino-boundary-audit-2026-09-05.md`、`docs/logs/2026-09-06-eino-native-mcp/`。 |
