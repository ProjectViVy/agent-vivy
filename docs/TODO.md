...
| RB-1 | 回退调研：回退怎么做、到底支不支持代码回退 | OPEN | 调研完成 2026-08-31（`docs/research/rollback-capability-research-2026-08-31.md`）：① Vivy 现状不支持任何代码回退，且当前形态下"不支持"是对的——文件工具只写每 run 隔离 scratch（`workspace_root`/`<runID>`），checkpoint 桥纯运行态无文件语义，旧内容只有 32KB 有损 diff 残影；② 承诺口径 = 文件级回退（version chain），**不做 git 语义回退**（git 归用户，agent 只对自己动过的文件负责）；③ 落点 = 主线内核（存储+写路径+审批三处核心），L1 存档+单文件恢复随 VC-1（bash 化后成为安全网前提），L2 会话级批量回退随 VC-3；④ 参照发现：Crush 版本链全库无 restore/undo 消费方（仅 TUI 展示面板）——Vivy 做了即领先。待拍板 O1..O6（研究 §6：保留策略/存储形态/存档范围/恢复治理/bash 边界），拍板后本行转 DONE |
