...
| TUI-USAGE-ACCOUNTING | 补齐 provider 调用的 route/source 归因与统一 cost-known 契约 | OPEN | 2026-09-05 token/cost 审计发现：child usage 缺 provider/model route 且未携 cached tokens；summary model usage 会按主 run route 归属；自动标题调用未入账；全局 stats 的 `cost_known` 是“至少一条可定价”而 sidebar 要求“全部可定价”；cached token 仍按普通 input 参考价估算。后续应引入 typed usage attribution（provider/model/source）、统一 all-priced 语义并明确辅助调用边界；不得把 partial/reference cost 当 provider 账单。相关路径 `internal/runtime/modelbroker.go`、`internal/app/worker.go`、`internal/runtime/compaction_middleware.go`、`internal/runtime/titler.go`、`internal/rpc/tokenstats.go`。 |
...
| TUI-VIEWPORT-N1-OPEN | 主聊天 viewport 的稳定锚点与超长历史缓存 | OPEN | 2026-09-05 已交付行级真实 viewport、follow/pause、PgUp/PgDn/Home/End、Crush 式非右栏滚轮 owner、overlay 隔离、会话切换复位及 ANSI/grapheme/窄窗口安全。后续非阻塞优化：当前 paused offset 是渲染行号而非 message/segment anchor，若旧位置上方内容继续增长会产生相对位移；每次 clamp/help/render 仍重复渲染整段历史；active session 被删除后 ID 会先切换、历史稍后加载的既有异步边界需建立显式 loading epoch；右栏当前需先点击取得 scroll owner，直接悬停滚轮仍会落到聊天区，后续若完全对齐 Crush 的 pointer-region wheel routing 应补区域路由与测试。相关路径 `sdk/tui/view`、`internal/tui/live.go`、`faces/tui/live.go`。 |
