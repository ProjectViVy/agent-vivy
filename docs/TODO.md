...
|---|---|---|---|
| PLG-1 | 插件扩展面收窄拍板：seam 四值闭集 + grants 八词闭集 + 单一 `Plugin` 接口（仅 Tools）使 Vivy 插件能力远窄于 DSH Cordis（~55 ctx 服务 key + 类型化事件）；`SeamProvider` 有名无实（内核零消费者）；候选 = ① SeamProvider 落地（provider 可插件供给）② hook/事件 seam（复用 `ToolHookChain`，钩子来源从 config 脚本扩到插件）——均不动编译期 pack/generation 审计范式 | OPEN | 2026-09-02 调研 `docs/research/plugin-model-dsh-vs-vivy-research-2026-09-02.md`：运行时装载/模型自写插件/插件进 Journal=有意差距不追（NG-11、SELF-EVOLVING-GATEWAY §366-388）；收窄项按性价比排序待拍板 |
...
| UI-COMPOSER | 聊天框剩余伪操作按钮接后端：附件 / AutoDream / "＋更多" / 思考模式 / 询问模式 | OPEN | 2026-09-02 可行部分交付：附件（VC-1g-2 链路已通）+ 思考模式端到端（`turn/start` `thinking` 参数 → runCtx → Anthropic `WithThinking` 注入，D9 元数据门控 `session/context.thinking_supported` 控制选择器可见性；OpenAI reasoning_effort 旋钮与'强制 off'未接，见 `docs/logs/2026-09-02-thinking-mode-d9/`）。余项：AutoDream 等 MEM-1（DEFERRED）；询问模式需内核语义提案；"＋更多"无对应后端面。执行模式 agent/plan 已接线（`RunMode` normal/plan） |
...
| FACE-TUI-1 | Packed `faces/tui` organ (F3) | OPEN | 2026-08-29 探路客户端 `vivy tui`（`internal/tui`）已能连驻留网关对话/审批；不是配方器官；默认双击仍是 web。2026-09-02 复核前置：F0 已采纳（D2）且 paperwork 已补（`docs/logs/2026-09-02-face-pack-f0-paperwork/`）；headless **功能**已以 `vivy run`（D11）存在但未做成配方器官；F3 真实剩余 = F1（无网页控制面：进程内 RPC 无 embed/无 listen 跑完对话+审批，`app` 组合拆 HTTP 监听耦合）+ FaceHost/SDK Face 契约 + 出厂 `faces/tui` 器官 + pack `face:` 配方键——按合同 PR 计划分 2–3 片实施，非单切片 |
...
| UI-CRON-P2 | cron 外发通道/补跑/at 表单入口 | OPEN | 2026-08-30 cron 闭环后留白：`payload.deliver/channel/to` 仅存储不生效（Vivy 无外发通道，等 CH-0）；错过的调度不补跑（catch-up）；cron 运行沿用会话默认审批策略，无独立治理面。2026-09-02 「at 表单入口」子项闭账：定时一次进 UI 表单（datetime-local 触发时间 + 未来校验 + onceAt 展示 + 编辑回填 + 离线 e2e 规格），行整体因补跑/外发仍等 CH-0 保持 OPEN。Filing: `docs/logs/2026-09-02-uicronp2-at-form/` |
...
| UI-CHAT-ACT | 消息编辑 / 回退 / 分叉启用 + 截断式重新生成 | OPEN | 2026-08-26 功能栏已移植（复制启用 + 重新生成=重发上一条用户输入开新回合，见 `docs/logs/2026-08-26-chat-message-actions/`）；编辑/回退/分叉为禁用占位。2026-09-02 **设计片交付**：`docs/architecture/JOURNAL-REWIND-AND-FORK.md`——逻辑截断标记（migration 020，不删行、读时折叠，压缩先例同族）+ `session/rewind`/`session/fork` 两 RPC + busy 闸 + UI 接线；实施切分 R1（内核回退）/R2（fork+编辑接线）/R3（打磨）。2026-09-02 **R1 内核交付**：`session_truncations` 表（sqlite migration020 + postgres）+ `TruncationStore` 契约 + CN-21 一致性用例；运行时 `RewindSession`（busy 闸/无效截点拒收/`session.truncated` 审计事件，tr_ 前缀）+ 单一过滤点 `effectiveSessionMessages`（模型上下文/trajectory/session/messages 三视图同源）；RPC `session/rewind`（NotFound/Conflict/InvalidParams 映射）+ `session.rewind` capability。UI 接线在 R2，fork 在 R2。Filing: `docs/logs/2026-09-02-chat-act-r1/` |
| UI-CHAT-TOOLBAR | 聊天框上方功能栏后端接线 + 状态持久化 | OPEN | 2026-08-29 权限三段（谨慎/智能/信任）已接 `session/set_permission`（见 `docs/logs/2026-08-29-sandbox-permission-presets/`）。2026-09-02 复核：执行模式已接线（UI-AUDIT-CHAT-MODE）；思考模式已接 D9 门控端到端（`docs/logs/2026-09-02-thinking-mode-d9/`）；附件已接 VC-1g-2 链路。余项：AutoDream（等 MEM-1）、桌面伙伴/语音（无内核能力）、思考/执行模式为逐回合选择不做会话持久化（Crush 语义） |
