...
**探路（2026-08-29，不是 F3）。** 仓库里有 `vivy tui`（`internal/tui`）：连已经在听的网关 `/rpc`，REPL 发 `turn/start`，流式打 `model.delta`，审批走 y/n。它是控制面客户端，不是出厂 `faces/tui`，不省略 web，不改默认无参启动。默认双击仍是网页网关。F3 仍要求 FaceHost + 配方点名 + 制品不含 `ui/dist`。
...
1. **出厂 `faces/` 是否独立 `go.mod`。** 推荐独立，保证网关世代无 TUI deps。同 module + 空注册表是较小的第一刀，但更易被误 import。
2. **默认提交的物种身体是否永远 `face: web`。** 推荐是。coding 世代是另一条配方，不替换日常双击的网关。
3. **网页与 TUI 能否点同一 Journal 同居。** 第一刀否（一代一张嘴）。多客户端同居是后切，且必须钉死审批 first-writer-wins。
4. **`headless` 遇到审批时的产品句子。** 推荐失败退出，不挂起等、不 yolo。需要产品确认。
