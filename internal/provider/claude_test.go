...
func TestClaudeRefRequiresKeyBeforeConstruction(t *testing.T) {
	...
	ref := testRef(t, testClaudeVendor("https://api.anthropic.com"))
...
func TestClaudeRefOutboundProtocol(t *testing.T) {
	...
	ref := testRef(t, testClaudeVendor(srv.URL))
	...
	// The vendor's endpoint enables prompt caching, so an ephemeral auto
	// breakpoint must ride the request (§8.5 acceptance: caching 生效).
...
func TestClaudeRefFallsBackToVendorDefaults(t *testing.T) {
	var body struct {
		Model string `json:"model"`
	}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if body.Model != "claude-sonnet-4-5" {
		t.Fatalf("outbound model = %q, want the vendor default_model", body.Model)
	}
}
...
func TestClaudeModelInfoKnownAndUnknown(t *testing.T) {
	...
	if empty, _ := ref.ModelInfo(context.Background(), ""); empty.ID != "claude-sonnet-4-5" {
		t.Fatalf("empty id must fall back to the vendor default, got %+v", empty)
	}
}

func TestCatalogForClaudeBackend(t *testing.T) {
	c := NewCatalog(testClaudeVendor("https://api.anthropic.com"))
