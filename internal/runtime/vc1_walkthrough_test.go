...
// ripgrep, repairs it via multiedit, and verifies the fix on disk through the
...
func TestVC1Walkthrough(t *testing.T) {
	...
	eng, err := NewEngine(ctx, NewScriptedModel(
		...
		schema.AssistantMessage("", []schema.ToolCall{{
			ID:       "call-wt-bash",
			Function: schema.FunctionCall{Name: tools.BashName, Arguments: `{"command":"rg PASS calc.sh"}`},
		}}),
