...
func TestFastTerminalBeforeRunReturnsIsDelivered(t *testing.T) {
	...
	run := func(ctx context.Context, sessionID domain.SessionID, _ string, _ []domain.Attachment, _ *domain.Provenance, prepare PrepareRunFunc) (domain.RunID, error) {
		...
		open, err := backend.ListOpenChannelDeliveries(ctx)
		if err != nil {
			return "", err
		}
		if len(open) != 1 || open[0].RunID != runID || open[0].State != storage.ChannelDeliveryArmed {
			t.Fatalf("delivery intent after prepare = %+v, want run %s armed", open, runID)
		}
