...
func TestLiveDynamicCommandsUseTypedCatalogAndExpansion(t *testing.T) {
	listCalls := 0
	handler := controlrpc.HandlerFunc(func(_ context.Context, _ *controlrpc.Peer, request controlrpc.Request) (any, *controlrpc.Error) {
		switch request.Method {
		case "session/list":
			return map[string]any{"sessions": []any{map[string]any{"id": "sess_1", "title": "one"}}}, nil
		case "session/messages":
			return map[string]any{"messages": []any{}}, nil
		case "commands/list":
			listCalls++
			if listCalls > 2 {
				return nil, &controlrpc.Error{Code: controlrpc.InternalError, Message: "catalog unavailable"}
			}
