...
func (b *Backend) AppendMessageIfAbsent(ctx context.Context, m domain.Message) (bool, error) {
	...
	result, err := tx.ExecContext(ctx,
		`INSERT OR IGNORE INTO messages (id, session_id, run_id, role, created_at, work_seq, content, tool_call_id, tool_name, tool_args, source, channel, chat_id, channel_message_id)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		m.ID, m.SessionID, m.RunID, string(m.Role), m.CreatedAt, int64(m.WorkSeq), m.Content,
...
func (b *Backend) listFileContexts(ctx context.Context, sessionID domain.SessionID, out []domain.Message, byID map[string]int) error {
	rows, err := b.db.QueryContext(ctx,
		`SELECT c.message_id, c.path, c.name, c.size, c.content
		 ...
		 WHERE m.session_id = ? ORDER BY c.message_id, c.position`, sessionID)
	if err != nil {
