...
// cutoff. GLOB keeps the underscore a literal (LIKE would treat it as a
// wildcard), and only the provenance pseudo-runs match — real run events
// are never touched.
func (b *Backend) PruneChannelInboundEvents(ctx context.Context, olderThan time.Time) (int, error) {
	res, err := b.db.ExecContext(ctx,
		`DELETE FROM run_events WHERE run_id GLOB 'chanin_*' AND created_at < ?`,
		olderThan.UnixMilli())
	if err != nil {
