...
// sqliteContinuitySessions returns the sorted, deduplicated set of sessions
// the admission transaction must hold: destination plus every referenced
// source. SQLite serializes writers on the single connection, so the sorted
// scan is the cross-backend lock-order contract rather than a lock upgrade.
func sqliteContinuitySessions(a storage.ContinuityAdmission) []domain.SessionID {
...
func sqliteLockContinuitySessions(ctx context.Context, tx *sql.Tx, a storage.ContinuityAdmission) error {
	...
	for _, id := range sqliteContinuitySessions(a) {
		var found string
...
func sqliteFindReceiptTx(ctx context.Context, tx *sql.Tx, sessionID domain.SessionID, operation, requestID string) (domain.ContinuityReceipt, bool, error) {
...
// sqliteReplayAdmission loads the originally committed startup events — the
// contiguous prefix ending at the receipt's event_seq — so a lost response
// returns the same committed startup view without re-running anything.
func sqliteReplayAdmission(ctx context.Context, tx *sql.Tx, receipt domain.ContinuityReceipt) (storage.ContinuityResult, error) {
	rows, err := tx.QueryContext(ctx,
		`SELECT run_id, seq, type, created_at, payload_version, payload FROM run_events
		 WHERE run_id = ? AND seq <= ? ORDER BY seq`, receipt.RunID, int64(receipt.EventSeq))
	if err != nil {
...
func sqliteCheckExpectations(ctx context.Context, tx *sql.Tx, expectations []storage.SourceExpectation) error {
	for _, e := range expectations {
		...
		if err := tx.QueryRowContext(ctx,
			`SELECT COUNT(*) FROM session_truncations WHERE session_id = ? AND reason IN ('rewind','edit')`,
...
func sqliteInsertContinuityEvent(ctx context.Context, tx *sql.Tx, e *domain.RunEvent, seq int64) error {
...
func sqliteInsertMessageTx(ctx context.Context, tx *sql.Tx, m domain.Message) error {
...
func sqliteFault(hook func(string) error, step string) error {
	if hook == nil {
		return nil
	}
	return hook(step)
...
// CommitContinuityRun commits the complete task admission atomically. Rows
// land in one transaction: destination user message, the accepted run row,
// the ordered startup events and the idempotency receipt.
func (b *Backend) CommitContinuityRun(ctx context.Context, a storage.ContinuityAdmission) (storage.ContinuityResult, error) {
	...
	if err := sqliteLockContinuitySessions(ctx, tx, a); err != nil {
		return storage.ContinuityResult{}, err
	}

	// Receipt lookup precedes every write so an identical replay returns the
	// original run without a second admission.
	existing, found, err := sqliteFindReceiptTx(ctx, tx, a.SessionID, a.Receipt.Operation, a.Receipt.RequestID)
	if err != nil {
		return storage.ContinuityResult{}, err
	}
	if found {
		...
		return sqliteReplayAdmission(ctx, tx, existing)
	}

	if err := sqliteCheckExpectations(ctx, tx, a.Expectations); err != nil {
		return storage.ContinuityResult{}, err
	}
	if err := sqliteInsertMessageTx(ctx, tx, a.Message); err != nil {
		return storage.ContinuityResult{}, err
	}
	if err := sqliteFault(a.TestFaultHook, "message"); err != nil {
		return storage.ContinuityResult{}, err
	}
	kind := a.Run.Kind
	if kind == "" {
	...
	if err := sqliteFault(a.TestFaultHook, "run"); err != nil {
		return storage.ContinuityResult{}, err
	}
	events := append([]domain.RunEvent(nil), a.Events...)
	for i := range events {
		if err := sqliteInsertContinuityEvent(ctx, tx, &events[i], int64(i+1)); err != nil {
			return storage.ContinuityResult{}, err
		}
	}
	if err := sqliteFault(a.TestFaultHook, "events"); err != nil {
		return storage.ContinuityResult{}, err
	}
	receipt := a.Receipt
	...
	if _, err := tx.ExecContext(ctx,
		`INSERT INTO continuity_receipts (session_id, operation, request_id, input_hash, run_id, event_seq) VALUES (?,?,?,?,?,?)`,
	...
	if err := sqliteFault(a.TestFaultHook, "receipt"); err != nil {
		return storage.ContinuityResult{}, err
	}
...
func (b *Backend) CommitContinuityOperation(ctx context.Context, m storage.ContinuityMutation) (storage.ContinuityResult, error) {
	...
	var owner string
	err = tx.QueryRowContext(ctx, `SELECT session_id FROM runs WHERE id = ?`, m.RunID).Scan(&owner)
	switch {
	...
	existing, found, err := sqliteFindReceiptTx(ctx, tx, m.SessionID, m.Receipt.Operation, m.Receipt.RequestID)
	if err != nil {
		return storage.ContinuityResult{}, err
	}
	if found {
		...
		var typ string
		err := tx.QueryRowContext(ctx,
			`SELECT run_id, seq, type, created_at, payload_version, payload FROM run_events WHERE run_id = ? AND seq = ?`,
	...
	if err := tx.QueryRowContext(ctx,
		`SELECT MAX(seq) FROM run_events WHERE run_id = ?`, m.RunID).Scan(&maxSeq); err != nil {
		return storage.ContinuityResult{}, fmt.Errorf("storage: read max seq: %w", err)
	}
	event := m.Event
	if err := sqliteInsertContinuityEvent(ctx, tx, &event, maxSeq.Int64+1); err != nil {
		return storage.ContinuityResult{}, err
	}
	if err := sqliteFault(m.TestFaultHook, "events"); err != nil {
		return storage.ContinuityResult{}, err
	}
	receipt := m.Receipt
	...
	if err := sqliteFault(m.TestFaultHook, "receipt"); err != nil {
		return storage.ContinuityResult{}, err
	}
	if err := tx.Commit(); err != nil {
