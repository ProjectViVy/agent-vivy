...
func readWorkEventsTx(ctx context.Context, tx *sql.Tx, sessionID domain.SessionID) ([]domain.WorkEvent, error) {
	rows, err := tx.QueryContext(ctx,
		"SELECT session_id, work_seq, kind, payload_version, request_id, request_hash, created_at, payload FROM session_work_events WHERE session_id = ? ORDER BY work_seq",
		sessionID)
	if err != nil {
		return nil, fmt.Errorf("storage: read work events in transaction %s: %w", sessionID, err)
	}
	defer func() { _ = rows.Close() }()
	return scanWorkEvents(rows)
}
...
func readGoalRunTx(ctx context.Context, tx *sql.Tx, id domain.RunID) (domain.Run, error) {
	var run domain.Run
...
func readGoalStartedTx(ctx context.Context, tx *sql.Tx, id domain.RunID) (domain.RunEvent, error) {
	var event domain.RunEvent
	var runID, typ string
	var seq int64
	err := tx.QueryRowContext(ctx,
		"SELECT run_id, seq, type, created_at, payload_version, payload FROM run_events WHERE run_id = ? ORDER BY seq LIMIT 1", id).
		Scan(&runID, &seq, &typ, &event.CreatedAt, &event.PayloadVersion, &event.Payload)
	if errors.Is(err, sql.ErrNoRows) {
