...
func (b *Backend) RecordFileMutation(ctx context.Context, sessionID domain.SessionID, runID domain.RunID, path string, oldContent, newContent []byte) error {
	...
	err = tx.QueryRowContext(ctx, `
		...
		WHERE session_id = ? AND path = ? ORDER BY version DESC LIMIT 1`,
	...
	if err := tx.QueryRowContext(ctx, `
		...
		WHERE session_id = ? AND path = ? ORDER BY version DESC LIMIT 1 OFFSET ?`,
	...
	if cutoff.Valid {
		if _, err := tx.ExecContext(ctx, `
			DELETE FROM file_versions WHERE session_id = ? AND path = ? AND version <= ?`,
