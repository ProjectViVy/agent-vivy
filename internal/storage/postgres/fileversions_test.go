...
var fileVersionSchemaSeq atomic.Uint64
...
func TestFileVersionChainSemantics(t *testing.T) {
	...
	schema := fmt.Sprintf("fv_%d_%d", time.Now().UnixNano(), fileVersionSchemaSeq.Add(1))
	...
	versions := func(t *testing.T, path string) []string {
		...
		rows, err := b.db.SQL.QueryContext(ctx,
			`SELECT content FROM file_versions WHERE session_id = $1 AND path = $2 ORDER BY version`, sid, path)
		if err != nil {
