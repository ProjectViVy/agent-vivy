...
func (b *Backend) SaveDefinition(ctx context.Context, record storage.WorkflowDefinitionRecord) error {
	...
	if _, err := tx.ExecContext(ctx, `
		...
		VALUES (?, ?, ?, ?, ?)`, record.ID, record.Rev, record.Hash, append([]byte(nil), record.Definition...), record.CreatedAt); err != nil {
...
func workflowUniqueViolation(err error) bool {
	...
	return errors.As(err, &pgErr) && pgErr.Code == "23505" || strings.Contains(strings.ToLower(err.Error()), "duplicate key")
}
