...
func (b *Backend) Append(ctx context.Context, commit storage.Commit) (domain.EventSeq, error) {
	...
	if _, err := tx.ExecContext(ctx, `SELECT pg_advisory_xact_lock(hashtextextended(?, 0))`, string(commit.RunID)); err != nil {
		return 0, fmt.Errorf("storage: lock run %s: %w", commit.RunID, err)
	}
