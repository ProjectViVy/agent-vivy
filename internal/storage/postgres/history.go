...
func (b *Backend) CaptureHistoryCut(ctx context.Context, ids []domain.SessionID) (storage.HistoryCut, error) {
	...
	rows, err := tx.QueryContext(ctx, `SELECT r.id, COALESCE(MAX(e.seq), 0)
		...
		WHERE r.session_id IN (`+postgresPlaceholders(len(args))+`) GROUP BY r.id ORDER BY r.id LIMIT 257`, args...)
	if err != nil { return storage.HistoryCut{}, fmt.Errorf("storage: capture history run ceilings: %w", err) }
	for rows.Next() { var id string; var seq int64; if err := rows.Scan(&id, &seq); err != nil { rows.Close(); return storage.HistoryCut{}, err }; cut.Runs = append(cut.Runs, storage.HistoryRunCut{RunID: domain.RunID(id), Seq: domain.EventSeq(seq)}) }
	if err := rows.Err(); err != nil { rows.Close(); return storage.HistoryCut{}, err }
...
func (b *Backend) QueryHistoryPage(ctx context.Context, cut storage.HistoryCut, after storage.HistoryPosition, options storage.HistoryQueryOptions) (storage.HistoryCandidates, error) {
	...
	if err != nil { return storage.HistoryCandidates{}, err }
	hidden, err := postgresHiddenHistoryPositions(ctx, tx, cut)
	if err != nil { return storage.HistoryCandidates{}, err }
	...
	for i, item := range meta {
		...
		if item.bytes > remaining { if remaining > 0 { out.BytesInspected += remaining }; if !hidden[item.sessionID][item.position] { out.Records = append(out.Records, postgresUnavailableHistoryCandidate(item)) }; out.ScanIncomplete = i+1 < len(meta); break }
		out.BytesInspected += item.bytes
		if hidden[item.sessionID][item.position] { continue }
		if item.bytes > limits.ResultItemBytes { out.Records = append(out.Records, postgresUnavailableHistoryCandidate(item)); continue }
		var body []byte
...
type postgresHistoryMetadataRow struct { id string; sessionID domain.SessionID; runID domain.RunID; role string; createdAt, position int64; bytes int }
...
func postgresHistoryMetadata(ctx context.Context, tx *sql.Tx, cut storage.HistoryCut, after storage.HistoryPosition, max int) ([]postgresHistoryMetadataRow, error) {
	clauses := make([]string, 0, len(cut.Sessions)); args := make([]any, 0, len(cut.Sessions)*2+3); number := 1
	for _, c := range cut.Sessions { clauses = append(clauses, fmt.Sprintf("(session_id = $%d AND position <= $%d)", number, number+1)); args = append(args, c.SessionID, c.Position); number += 2 }
	query := `SELECT id, session_id, run_id, role, created_at, position, octet_length(content) FROM messages WHERE (`+strings.Join(clauses, " OR ")+		`)`
	if !after.IsZero() { query += fmt.Sprintf(` AND (session_id > $%d OR (session_id = $%d AND position > $%d))`, number, number+1, number+2); args = append(args, after.SessionID, after.SessionID, after.Position); number += 3 }
	query += fmt.Sprintf(` ORDER BY session_id, position LIMIT $%d`, number); args = append(args, max)
	rows, err := tx.QueryContext(ctx, query, args...)
	if err != nil { return nil, fmt.Errorf("storage: enumerate history metadata: %w", err) }
	defer rows.Close()
	var out []postgresHistoryMetadataRow
	for rows.Next() { var item postgresHistoryMetadataRow; var sid, rid string; if err := rows.Scan(&item.id, &sid, &rid, &item.role, &item.createdAt, &item.position, &item.bytes); err != nil { return nil, err }; item.sessionID, item.runID = domain.SessionID(sid), domain.RunID(rid); out = append(out, item) }
	return out, rows.Err()
}

func postgresHiddenHistoryPositions(ctx context.Context, tx *sql.Tx, cut storage.HistoryCut) (map[domain.SessionID]map[int64]bool, error) {
	...
	for _, c := range cut.Sessions {
		...
		rows, err := tx.QueryContext(ctx, `SELECT cutoff_message_id, tail_message_id FROM session_truncations WHERE session_id = $1 AND (reason = $2 OR reason = $3) ORDER BY id`, c.SessionID, storage.TruncationRewind, storage.TruncationEdit)
		...
		for _, marker := range markers { var first, last int64; a := tx.QueryRowContext(ctx, `SELECT position FROM messages WHERE id = $1 AND session_id = $2`, marker[0], c.SessionID).Scan(&first); z := tx.QueryRowContext(ctx, `SELECT position FROM messages WHERE id = $1 AND session_id = $2`, marker[1], c.SessionID).Scan(&last); if a == nil && z == nil && first <= last { for p := first; p <= last; p++ { hidden[c.SessionID][p] = true } } }
	}
	return hidden, nil
}

func postgresUnavailableHistoryCandidate(item postgresHistoryMetadataRow) storage.HistoryCandidate { return storage.HistoryCandidate{Ref: domain.SourceRef{SessionID: item.sessionID, RunID: item.runID, MessageID: item.id, Kind: string(domain.SourceKindMessage), CreatedAt: item.createdAt}, Author: item.role, Unavailable: true, Truncated: true} }
func postgresSessionArgs(ids []domain.SessionID) []any { out := make([]any, len(ids)); for i := range ids { out[i] = ids[i] }; return out }
func postgresPlaceholders(n int) string { parts := make([]string, n); for i := range parts { parts[i] = fmt.Sprintf("$%d", i+1) }; return strings.Join(parts, ",") }
