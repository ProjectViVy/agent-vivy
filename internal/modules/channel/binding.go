...
func bindProviders(providers []channelport.ChannelProvider, grants map[string][]module.GrantBinding, configured channelcontract.Config) ([]channelport.Channel, error) {
	...
	out := make([]channelport.Channel, 0, len(providers))
...
func (c *providerChannel) Start(ctx context.Context, host channelport.Host) error {
...
func (c *providerChannel) Send(ctx context.Context, msg channelport.OutboundMessage) ([]string, error) {
	if c.instance == nil {
...
func (host grantedChannelHost) HTTP() *http.Client {
	...
	if !ok {
		return &http.Client{Transport: roundTripperFunc(func(*http.Request) (*http.Response, error) {
			return nil, channelport.ErrDenied
		})}
	}
	base := host.Host.HTTP()
	if base == nil {
...
func (host grantedChannelHost) PublishInbound(ctx context.Context, message channelport.InboundMessage) error {
...
func (host grantedChannelHost) Media() channelport.MediaStore { return host.Host.Media() }
