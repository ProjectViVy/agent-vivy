...
export function MessageBubble({ message, reasoning, streaming }: { message: Message; reasoning?: string; streaming?: boolean }) {
  ...
  if (message.role === 'tool') return <div className="mx-auto my-3 max-w-2xl min-w-0 rounded-xl border bg-muted/40 p-3 text-sm"><div className="mb-1 text-xs font-medium text-muted-foreground">工具结果</div><pre className="overflow-x-auto whitespace-pre-wrap break-words">{message.content}</pre></div>;
