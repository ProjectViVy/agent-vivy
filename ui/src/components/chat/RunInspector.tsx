...
export function RunInspector() {
  ...
  return <Tabs value={tab} onValueChange={setTab} className="flex h-full flex-col"><TabsList className="mx-4 mt-4 grid grid-cols-4"><TabsTrigger value="run">{t('runInspector.currentRun')}</TabsTrigger><TabsTrigger value="background">{t('runInspector.background', { count: background.length })}</TabsTrigger><TabsTrigger value="children">{t('runInspector.children', { count: children.length })}</TabsTrigger><TabsTrigger value="review">{t('runInspector.review', { count: runReviews.length })}</TabsTrigger></TabsList>
    <div className="flex-1 overflow-auto p-4"><TabsContent value="run" className="mt-0 space-y-4">{run ? <><div className="rounded-lg border p-3 text-sm"><div className="flex items-center justify-between"><code>{run.id}</code><Badge>{run.status}</Badge></div><div className="mt-2 text-muted-foreground">{t('hostLabels.session', { id: run.session_id })}</div></div><div><h3 className="mb-2 text-sm font-medium">{t('runInspector.events')}</h3><div className="space-y-1">{events.slice().reverse().map((event) => <div key={event.seq}><button type="button" className={`flex w-full items-center gap-2 rounded p-2 text-left text-xs hover:bg-muted ${openSeq === event.seq ? 'bg-muted' : ''}`} aria-expanded={openSeq === event.seq} onClick={() => setOpenSeq(openSeq === event.seq ? null : event.seq)}><span className="w-10 text-muted-foreground">#{event.seq}</span><span>{event.type}</span></button>{openSeq === event.seq ? <pre className="mb-1 max-h-48 overflow-auto whitespace-pre-wrap break-words rounded bg-muted/60 p-2 font-mono text-[11px] leading-4">{JSON.stringify(event.payload ?? null, null, 2)}</pre> : null}</div>)}</div>{attachedRefs.length > 0 ? <div><h3 className="mb-2 text-sm font-medium">{t('runInspector.references', { count: attachedRefs.length })}</h3><div className="space-y-1">{attachedRefs.map((row) => <div key={row.id} className="rounded border p-2 text-xs"><code>{row.reference.id}</code> <span className="text-muted-foreground">{row.reference.source_session_id} · {row.reference.items.length} · {row.reference.origin}</span></div>)}</div></div> : null}</div></> : <p className="py-10 text-center text-sm text-muted-foreground">{t('runInspector.noRun')}</p>}</TabsContent>
    ...
    <TabsContent value="review" className="mt-0 space-y-3">{runReviews.length ? runReviews.map((review) => <ReviewCard key={review.id} review={review} busy={reviewBusyIds.includes(review.id)} onRespond={(response) => respondReview(review.id, response)} />) : <p className="py-10 text-center text-sm text-muted-foreground">{t('runInspector.noReviews')}</p>}</TabsContent>
