...
export function ChatView({ sessionId }: { sessionId: string }) {
  ...
  return <div className="flex h-full flex-col"><ScrollArea className="flex-1"><div className="mx-auto max-w-4xl p-4">
  ...
  {pending ? <div className="border-t border-amber-500/30 bg-amber-500/10 px-4 py-3"><div className="mx-auto flex max-w-3xl items-start gap-3"><AlertTriangle className="mt-0.5 h-4 w-4 text-amber-600"/><div className="min-w-0 flex-1 text-sm"><div className="font-medium">预检{pending.result.status === 'blocked' ? '已阻止本次运行' : '发现警告'}</div>{[...(pending.result.blockers ?? []), ...(pending.result.warnings ?? [])].map((item) => <p key={item} className="mt-1 text-muted-foreground">{item}</p>)}</div><Button variant="ghost" size="sm" onClick={() => setPending(null)}>取消</Button>{pending.result.status !== 'blocked' ? <Button size="sm" onClick={() => void continueRun()}>继续</Button> : null}</div></div> : null}
