/**
 * 轨迹工具函数的测试夹具（本地静态示例）。
 * 仅被 `trajectory-utils.test.ts` 引用：面板已切换真实
 * `trajectory/session` RPC（UI-TRAJECTORY-DEMO），生产链路不再使用本模块。
 *
...
export type { TrajectoryCellKind, TrajectoryRecord, TrajectoryRequest, TrajectoryTimeRange, TrajectoryTimelineMode, TrajectoryTokens };
