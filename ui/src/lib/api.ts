...
/** 运行时接受的封闭消息来源词汇（CH-C1-N4）。 */
export type MessageSource = 'ui' | 'channel' | 'headless';
/** 消息世界入口出处（CH-C1）：仅 channel 轮携带，ui 轮不出该字段。 */
export interface MessageProvenance { source: MessageSource; channel?: string; chat_id?: string; channel_message_id?: string }
