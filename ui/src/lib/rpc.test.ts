...
describe('RpcClient bootstrap', () => {
  ...
  it('preserves the additive code mode capability from initialize', async () => {
    class FakeWebSocket {
      onopen: (() => void) | null = null;
      onerror: (() => void) | null = null;
      onmessage: ((message: { data: string }) => void) | null = null;
      ...
      send(payload: string): void {
        ...
        queueMicrotask(() => this.onmessage?.({
          data: JSON.stringify({
            jsonrpc: '2.0',
            id: request.id,
            result: { protocol_version: 'vivy.rpc.v1', capabilities: ['session.read'], code_mode_available: true },
          }),
        }));
      }
      close(): void { this.onclose?.(); }
    ...
    expect(client.capabilities.code_mode_available).toBe(true);
    expect(getRpcCapabilitiesSnapshot()).toEqual({ protocol_version: 'vivy.rpc.v1', capabilities: ['session.read'], code_mode_available: true });
  });
});
