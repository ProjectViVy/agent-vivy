...
function InstalledRow({ item, t, langKey, canReveal, revealLabel, onOpenDetail, onReveal, onUninstall, onRestart }: {
  item: InstalledItem
  t: Translate
  langKey: LocaleId
  ...
  return h('li', {
    ...
    h('div', { className: styles.rowTitleLine },
      ...
      // 行操作：重启（dsh 插件待重启时）/ 详情 / 卸载 —— 按钮点击不触发行打开详情
...
function RowList({ items, t, langKey, canReveal, revealLabel, onOpenDetail, onReveal, onUninstall, onRestart }: {
  items: InstalledItem[]
  t: Translate
  langKey: LocaleId
  canReveal: boolean
...
export function InstalledView({ items, t, langKey, platform, onOpenDetail, onReveal, onUninstall, onRestart }: {
  items: InstalledItem[]
  t: Translate
  langKey: LocaleId
  ...
  const rowListProps = {
    ...
    onOpenDetail, onReveal, onUninstall, onRestart,
  }
