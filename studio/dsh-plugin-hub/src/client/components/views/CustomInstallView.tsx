 ...
 * git 框只认 GitHub 地址，命令框只认官方 `dsh plugin --profile <name> add <target>`
 * （--profile 必填，无 -p 简写；本构建禁用 update 动词，无任何升级路径）。
...
/** DSH 命令行前缀（只认 `dsh plugin --profile <name> add <target>`）—— 提取目标。
 * 官方 CLI（apps/cli/src/args.ts，commander）：--profile 为必填长选项（无 -p 简写，省略即报错），
 * 支持 `--profile=<name>` 等号形式；命令大小写不敏感。update/remove 不计入（本构建无升级路径）。
...
/** 解析 DSH 命令：返回目标；非官方 add 命令返回 null（交给格式错误提示）。 */
function parseDshCommand(raw: string): string | null {
...
export function CustomInstallView({ t, onInstallCustom, enableNpm, enableGit, enableDsh, onOpenSettings, profile }: {
  ...
  const submitCmd = () => {
    ...
    if (!raw) return
    const target = parseDshCommand(raw)
    if (target === null) { setCmdError(t('dshCmdInvalid')); return }
    // 只认 add：剥成裸目标发给服务端（本构建无 update 动词）
    onInstallCustom(target)
    setCmdQuery('')
    setCmdError('')
  }
  ...
  return h('div', { className: styles.root },
    ...
    h('div', { className: styles.installCards },
      ...
      // 命令卡片：粘贴官方 dsh plugin 命令即装 —— 只认完整 add 命令
      // （--profile 必填，可含 = 等号，动词 add，剥成裸目标走安装流程）
