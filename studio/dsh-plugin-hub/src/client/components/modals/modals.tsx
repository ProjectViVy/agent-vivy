    ...
    h('div', { className: styles.modal, role: 'dialog', 'aria-modal': 'true' },
      h('div', { className: styles.modalHead },
        ...
        : h('div', { className: styles.modalBody },
          ...
          // 手动命令条：仅命令行安装（pnpm add <target>）与全局 npm 安装（npm install -g <pkgs>）
          // 展示真实命令、整条点击即复制；目录插件由 VIVY-STUDIO-PLUGIN-HUB 自动安装
          // （vivy-source 模式克隆到源码树），不再展示/复制 dsh plugin 终端命令，避免误导。
          h('div', {
            className: styles.modalCmd,
            onClick: globalNpm !== undefined && globalNpm.length > 0 || customTarget ? onCopy : undefined,
            title: globalNpm !== undefined && globalNpm.length > 0 || customTarget ? t('copyInstallCommand') : undefined,
            role: 'button',
            tabIndex: 0,
          },
            h('span', { className: styles.modalCmdText },
              globalNpm !== undefined && globalNpm.length > 0
                ...
                : customTarget ? `pnpm add ${customTarget}` : t('vivyInstallCmdNote')),
            globalNpm !== undefined && globalNpm.length > 0 || customTarget
