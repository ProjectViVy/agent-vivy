...
import { runVivySourceMutation } from './vivy-source.ts'
...
export async function runPluginMutation(options: {
  ...
  // Vivy Studio source-tree install path: clone GitHub plugins into the Vivy
  // repo under studio/ and register them as file: bundles. Takes precedence
  // over the generic `dsh plugin` path when enabled.
  if ((options.globalNpm ?? []).length === 0) {
    const vivyResult = await runVivySourceMutation(options)
    if (vivyResult !== null) return vivyResult
  }
