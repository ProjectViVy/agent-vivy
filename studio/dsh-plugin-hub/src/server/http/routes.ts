...
import { isVivySourceInstallEnabled } from '../services/install/vivy-source.ts'
    ...
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          for (const key of ['enableNpmInstall', 'enableGitInstall', 'enableDshInstall', 'npmRegistry', 'proxy', 'logPath', 'vivySourceInstall'] as const) {
    ...
    webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          ...
          // Vivy Studio source-tree install path: GitHub repos are cloned and npm
          // packages are unpacked into the Vivy repo under studio/ (two flows).
          // Both skip npm-resolution and git-distribution preflight because the
          // source is installed and built locally.
          const vivySource = isVivySourceInstallEnabled(profile) && (repoTarget !== null || validPackageName(rawRepo))
          ...
          if (source !== 'catalog') {
            ...
            if (isGitChannel && !settings.enableGitInstall && !vivySource) {
              sendJson(response, 403, { error: 'git installs are disabled by the security settings' })
              return
            }
            if (!isGitChannel && !settings.enableNpmInstall && !vivySource) {
              sendJson(response, 403, { error: 'npm installs are disabled by the security settings' })
              return
            }
          }
