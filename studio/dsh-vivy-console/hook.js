...
// app bundle loads, so it captures console output, page errors, and
// WebSocket (JSON-RPC) traffic from the very first moment, and relays it to
// the Studio console tab via postMessage. Same-origin with the Studio page
// (the proxy facade runs on the Studio origin).
//
// Also answers parent-tab commands: "ping" and "evaluate" (run a JavaScript
  ...
  function post(type, payload) {
    try {
      window.parent.postMessage(
        { __vivyConsole: HOOK_TAG, type: type, payload: payload, url: location.href, t: Date.now() },
        "*",
      )
    } catch (e) {
      /* parent gone — ignore */
