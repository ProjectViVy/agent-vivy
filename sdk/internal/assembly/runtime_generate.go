...
func GenerateRuntimeAssembly(plan AssemblyPlan, packageName string) ([]byte, error) {
	...
	var channelFactoryModuleID, channelFactoryConstructor string
	for index := range modules {
		...
		if resolved.Binding.ChannelFactoryConstructor != "" {
			channelFactoryModuleID = resolved.Descriptor.Module.ID
			channelFactoryConstructor = resolved.Binding.ChannelFactoryConstructor
		}
	}
	...
	if channelFactoryModuleID != "" {
		fmt.Fprintf(&source, "\t\tChannelFactory: %s.%s(),\n", aliases[channelFactoryModuleID], channelFactoryConstructor)
	}
	...
	// These optional accessors are emitted only when the corresponding Source
	// Port is selected. The app consumes them through an optional interface so
	// a minimal overlay contains no source package imports, fields, manifest
	// edges, or source code paths at all.
	if hasContextSources {
...
func validateChannelFactoryBinding(resolved ResolvedModule) error {
	id := resolved.Descriptor.Module.ID
	binding := resolved.Binding
	providesChannelHost := descriptorProvides(resolved.Descriptor, "core/channel-host@v1")
	if binding.ChannelFactoryConstructor != "" {
		if !providesChannelHost {
			return fmt.Errorf("Channel factory constructor on %s requires core/channel-host@v1", id)
		}
		if id != "vivy/channel-host" {
			return fmt.Errorf("Channel factory constructor is reserved for canonical vivy/channel-host, got %s", id)
		}
		if binding.Constructor != "" {
			return fmt.Errorf("canonical Channel host %s cannot have both lifecycle and Channel factory constructors", id)
		}
		if !goIdentifierPattern.MatchString(binding.ChannelFactoryConstructor) {
			return fmt.Errorf("invalid Channel factory constructor for %s", id)
		}
	}
	if id == "vivy/channel-host" && providesChannelHost && binding.ChannelFactoryConstructor == "" {
		return fmt.Errorf("canonical Channel host %s is missing Channel factory constructor", id)
	}
	return nil
}
