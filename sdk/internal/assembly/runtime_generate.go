...
func GenerateRuntimeAssembly(plan AssemblyPlan, packageName string) ([]byte, error) {
	...
	for _, resolved := range modules {
		for _, provided := range resolved.Descriptor.Provides {
			if provided.Port == "std/tool-world@v1" && provided.ID == "mcp" {
				if !resolved.Binding.MCPHostProvider {
					return nil, fmt.Errorf("MCP ToolWorld mcp requires typed MCPHostProvider binding on %s", resolved.Descriptor.Module.ID)
				}
				if !descriptorProvides(resolved.Descriptor, "core/mcp-host@v1") {
					return nil, fmt.Errorf("MCP ToolWorld mcp requires core/mcp-host@v1 from %s", resolved.Descriptor.Module.ID)
				}
				hasMCPHostProvider = true
			}
	...
	var maskProvider *ResolvedModule
	for index := range modules {
		resolved := &modules[index]
		hasMaskPort := descriptorProvides(resolved.Descriptor, "core/mask-service@v1")
		if resolved.Binding.MaskFactory != "" && !hasMaskPort {
			return nil, fmt.Errorf("MaskFactory binding on %s does not provide core/mask-service@v1", resolved.Descriptor.Module.ID)
		}
		if !hasMaskPort {
			continue
		}
		if resolved.Binding.MaskFactory == "" {
			return nil, fmt.Errorf("mask-service Provider %s is missing typed MaskFactory binding", resolved.Descriptor.Module.ID)
		}
		if !goIdentifierPattern.MatchString(resolved.Binding.MaskFactory) {
			return nil, fmt.Errorf("invalid MaskFactory binding %q for %s", resolved.Binding.MaskFactory, resolved.Descriptor.Module.ID)
		}
		if maskProvider != nil {
			return nil, fmt.Errorf("duplicate core/mask-service@v1 Provider in %s and %s", maskProvider.Descriptor.Module.ID, resolved.Descriptor.Module.ID)
		}
		maskProvider = resolved
	}
	...
	if maskProvider != nil {
		source.WriteString("\t\"agent-vivy/internal/moduleport\"\n")
	}
	if hasContextSources {
		source.WriteString("\t\"agent-vivy/sdk/port/contextsource\"\n")
	}
	if hasSkillSources {
	...
	if maskProvider != nil {
		source.WriteString("\tMaskFactory moduleport.MaskFactory\n")
	}
	source.WriteString("\tProviderProfiles []providerprofile.Provider\n")
	if hasContextSources {
		source.WriteString("\tContextSources []contextsource.Provider\n")
	}
	if hasSkillSources {
	...
	for _, resolved := range modules {
		...
		for _, provided := range resolved.Descriptor.Provides {
			...
			case "std/control-action@v1":
				actionIDs = append(actionIDs, provided.ID)
	...
	grantExpr := func(grants []EffectiveGrant) string {
		...
		for _, grant := range grants {
			fmt.Fprintf(&out, "{Name: module.Grant(%q), Constraints: map[string][]string{", grant.Name)
	...
	for _, resolved := range modules {
		...
		for _, p := range resolved.Descriptor.Provides {
			...
			hasAction = hasAction || p.Port == "std/control-action@v1"
		...
		if hasAction {
			ids := make([]string, 0)
			for _, p := range resolved.Descriptor.Provides {
				if p.Port == "std/control-action@v1" {
					ids = append(ids, p.ID)
				}
			}
			idsExpr := "[]string{"
			for _, id := range ids {
				idsExpr += strconv.Quote(id) + ","
			}
			idsExpr += "}"
			providersExpr := "[]controlaction.Provider{" + call + "}"
			if resolved.Binding.ProviderCollection {
				providersExpr = call
			}
			actionExpr = append(actionExpr, fmt.Sprintf("controlaction.ProviderSet{ModuleID: %q, AllowedIDs: %s, Providers: %s, EffectiveGrants: %s}", resolved.Descriptor.Module.ID, idsExpr, providersExpr, grantExpr(resolved.EffectiveGrants)))
		}
		if hasProfile {
			profilesExpr = append(profilesExpr, providerExpression{call: call, collection: resolved.Binding.ProviderCollection})
		}
		if hasContextSource {
			contextSourcesExpr = append(contextSourcesExpr, providerExpression{call: call, collection: resolved.Binding.ProviderCollection})
		}
		if hasSkillSource {
	...
	writeProviders := func(field, typeName string, providers []providerExpression) {
		if len(providers) == 0 {
			return
		}
		expr := "[]" + typeName + "{}"
		for _, provider := range providers {
			if provider.collection {
				expr = "append(" + expr + ", " + provider.call + "...)"
			} else {
				expr = "append(" + expr + ", " + provider.call + ")"
			}
		}
	...
	writeProviders("ProviderProfiles", "providerprofile.Provider", profilesExpr)
	if hasContextSources {
		writeProviders("ContextSources", "contextsource.Provider", contextSourcesExpr)
	}
	if hasSkillSources {
	...
	if len(actionExpr) > 0 {
		fmt.Fprintf(&source, "\t\tActionSets: []controlaction.ProviderSet{%s},\n", strings.Join(actionExpr, ","))
	}
	...
	// These optional accessors are emitted for generated source projections. The
	// app consumes them through optional interfaces so a minimal overlay carries
	// no source package imports or fields for omitted capabilities.
	if hasContextSources {
	...
	// Keep the app's composition root independent from the optional generated
	// internal/moduleport import. The selected Assembly returns its typed
	// factory through this opaque accessor; a maskless Assembly returns nil and
	// carries no mask field or provider import.
	if maskProvider != nil {
